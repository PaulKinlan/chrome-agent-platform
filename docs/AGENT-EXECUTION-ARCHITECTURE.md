...
**Status:** DECIDED + **Phases 1–4 all implemented and shipped** (`0.2.308`–`0.2.310`, 2026-08-27). Owner chose **per-agent SHARED WORKERS** (MessagePorts passed to clients, SW-hop bootstrap). Phase 1 = the offscreen host + per-agent worker shell + SW alive-set; Phase 2 = the agent-do loop moved into the worker with authority staying in the SW; Phase 3 = durable progress/result/journal routes; Phase 4 = the UI port client with live redacted progress. (Phase 4 also shipped a SW-owned single-driver browser-command lease; it deadlocked the Settings toggle against runs and runs against revoke, and was removed by CAP-FB-20260830-BROWSER-LEASE-DEADLOCK-01 — destructive commands are gated by the browser-control grant + run fence.)
