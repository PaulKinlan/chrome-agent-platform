...
| C11 | ~~`scripts/live-every-tab.ts`~~ (RESOLVED 2026-09-26, bead `f99r` — **KEEP**) | The premise is gone: `2ypf` is CLOSED — MERGED @ `5ab96187`, and the script now kills the whole Chromium process tree by its unique profile marker (`killProcessTree` from `scripts/lib/process-tree.ts`). `tests/process-tree-cleanup.test.ts` pins that call as the 2ypf source contract: it reds if the parent-only kill pattern returns. Retained as the manual live check the harness registry records — it needs a real `GEMINI_API_KEY` and exits on its own verdict, and removal would delete the only live 30-tab sourced-digest check for RUN-BUDGET-EVERY-ITEM-01. |
