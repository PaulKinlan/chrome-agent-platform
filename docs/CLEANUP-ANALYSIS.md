...
| C2 | ~~`scripts/check-tasks.mjs` + `scripts/check-tasks-baseline.json` + `check:tasks`~~ (RESOLVED 2026-09-06, bead `raou` @ `a3a41062`, `i845` @ `c070352e`) | The gate enforcing the schema of retired TASKS.md was removed; `test:all` now starts cleanly with `check:vocabulary` and no dangling command remains. | Verified: `tests/package-scripts-exist.test.ts` (4/4 green); `npm run check:vocabulary` and `test:all` runnable. |
...
| C7 | ~~`scripts/p0-repro.ts` (+ registry entry)~~ **REMOVED 2026-09-26** (bead `rbtl`) | Red at the 2026-09-02 re-inventory ("a repro script for a P0 that has since moved on"). The P0 is the 2026-08-28 task-lifecycle emergency and it is closed: `n8dx` landed @ `0a4adbb2` (v0.3.299; `git merge-base --is-ancestor 0a4adbb2 origin/main` passes), `6arx` closed on owner retest, `b6t` closed — and the contract the harness diagnosed is pinned by `tests/task-lifecycle-contract.test.ts` plus source pins. Removed with its `harness-registry` row (git history keeps the script). | Verified: `tests/harness-registry.test.ts` (file↔entry both directions), `tests/task-lifecycle-contract.test.ts`, `tests/docs-process-truth.test.ts`, `npm run test:changed`, full `npm test`. |
...
| C15 | ~~`extension/lib/profile-store.js`~~ (RESOLVED 2026-09-25, bead `xtwv` — RETAINED and documented; decision: not orphaned) | The reachability framing was out of date: the SHIPPED `form-filler` skill (skill-registry.js; Data Wrangler + three other agent templates) fills fields "from the user's stored profile in memory", and this file is that substrate — schema-validated, grant-gated, write-audited profile store on OPFS/memory (dptw phase 2; hardened through rounds 2/3/5). docs/AGENT-PRODUCT-GAPS.md now names it as the persona's storage layer, and the reachability annotation carries the real reason instead of "only its test imports it". | Verified: check-reachability green with the new annotation; full `npm test` green. |
