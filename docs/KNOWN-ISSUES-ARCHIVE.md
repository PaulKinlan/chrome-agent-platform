# Known Issues — ARCHIVE (closed 2026-08-28)
...
### [Live lazy cutover candidate; fresh loaded-MV3 and independent release review pending] Co-do-style Wasm/tool OS — `CAP-FB-20260822-WASM-TOOL-PLATFORM-01`
- CAP has a public bounded metadata catalog and deterministic lexical index. The 0.2.180 candidate replaces eager provider maps with exactly two fixed AI-SDK definitions, `search_tools` and `execute_tool`, and binds single-use references to run/task/agent/origin/document/run-generation/catalog/source/closure/package/capability/permission/grant identity. Provider definition bytes remain constant at 20/100/1000 rows; non-selected definitions never cross. Search authorizes nothing and cannot install, approve, request permission, create a grant, or execute. Execute atomically claims a returned reference and revalidates immutable identity plus live permission/grant/source/run/document authority before validation, before dispatch, and after dispatch. Existing source closures remain the only dispatch authority. Bundled Wasm rows remain disabled catalog-only metadata with no reference or closure. Source/unit/build/package gates are required here; because Chrome is prohibited for this finisher, fresh loaded-MV3 denial/revoke/race acceptance remains an explicit blocker to merge/DONE.
- A source-only OPFS wrapper owns strict per-job roots, hash-verified write-once inputs, journaled byte/file reservations, trusted-anchor recovery, bounded idempotency state, validated terminal orphan GC and content-digest-bound keyed artifact-WAL promotion. A separate bundled-only package authority candidate validates duplicate-free canonical manifests, immutable inventory/CAS digests, measured Wasm framing/import/memory ceilings and SBOM/licence/build provenance, then journals record admission/update/revocation through exact-generation `__wasmTx`; signer metadata remains explicitly unverified. Its prior eight-character import grammar made truthful `wasi_snapshot_preview1` declarations impossible. The review correction separates the eight-module count from a 64-byte ASCII module-name bound and permits only exact `wasi_snapshot_preview1` in `allowed`; arbitrary `env`, typos, Unicode, overlong names and wildcard allows fail closed, while `disallowed` may contain `*` or bounded module names. This does not admit the reconstructed 39-tool set: its Apache-2.0-root versus MIT-metadata contradiction remains blocking. A third unreachable source candidate validates strict Unicode code-change documents, binds producer/run/input/base/result identity, preflights bounded CAS retention, retains and re-verifies bytes only through digest-keyed artifact WAL creates, and derives bounded non-authoritative text views. Apply/reject/undo are unavailable stubs: no owner approval, route or workspace mutation exists. These authorities expose no provider, install, package-owner, Worker or execution route, and no Wasm binary ships. The exploratory loaded-MV3 Wasm/offscreen/fresh-Worker/OPFS probe independently passed Gate 0, but does not ship product behavior. A new pure source candidate defines the frozen WASI P1 job/context/quota/FD/errno/rights contract and complete initial synchronous host-call table over injected memory/workspace adapters, including `fd_tell` and realtime-clock `ENOTSUP`; it remains unreachable and constructs no OPFS handle, Worker, route, byte load, instance or execution. Gate 2 integration and review still block use. The wider platform remains open: signer trust, fresh-Worker integration/termination, owner-approved diff mutation, Chrome lazy tools, Tool Library, owner install, bundled tools, spreadsheets, and abuse gates.
- Chrome Web Store remotely hosted code treatment for uploaded/downloaded Wasm is unresolved. Store mode remains bundled-reviewed-executable-only. The credential-free packaging boundary requires explicit `--target=store`, a canonical v2 commit/source/output/target marker, exact extension CSP equality, zero unmanifested Wasm, no alternate Worker literal, and no statically visible remote code-loading sink over actual tracked/generated package bytes before and after the unchanged exact-SHA archive path. Marker target is an intent/mismatch declaration only; the scanner classifies content. This does not make owner-selected packages Store-safe, execute or admit Wasm, or replace written policy clearance; owner-selected packages remain a distinct unpacked/enterprise/developer lane.
...
- **Package archive freshness (public deterministic marker; Store target successor in review):** public `093757fea4bee236f6b9038789ad4a67bd1f3b7a` derives an exact tracked-plus-generated regular-file inventory, keeps owner/time custody private, writes canonical commit/indexed-source/output marker bytes, verifies a fresh same-directory temp ZIP by names/hashes/extraction, and atomically replaces the final. The Store successor migrates that marker to v2 with `target:"store"`, rejects legacy/cross-target markers, and independently scans actual package bytes; same-source markers and ZIPs remain byte-identical. No Store policy approval is inferred.
...
- **CAS version-scoping refinements:** VERIFIED CLOSED (chrome-agent-platform-k8u, 2026-09-05): the version-scoped CAS in the memory + journal compensation path is behaviorally covered by tests/memory.test.ts (version-scoped CAS, identical-value ABA, journal ABA + generation-mismatch compensation, monotonic deleted-key versions); reverting the version check in `compareAndSet` turns all four RED, restoring returns GREEN. No residual ABA window: versions are monotonic per key, never reused, survive delete→rewrite.
- **First-message generation acceptance:** VERIFIED CLOSED (k8u): tests/k8u-concurrency-residuals.test.ts proves a generationless FIRST sync/invoke fails closed on a fresh bridge (maxGen = -Infinity), nothing arms, and a well-formed first sync still succeeds; reverting the null-generation guard turns the test RED.
- **runGenCells per-run isolation:** VERIFIED CLOSED (k8u): the cells now come from the per-build `createRunGenCellBook()` (lib/run-gen-cells.js) that buildOrchestrator uses; the k8u test proves concurrent same-origin builds hold distinct cells and a later build's bind can never repoint an earlier build's cell; reverting the book to a module-global map turns the test RED.
- **MAIN cancel tombstones bounded:** VERIFIED CLOSED (k8u): there ARE no tombstones — cancellation is the immutable monotonic `cancelEpoch` (two scalars + the timer-bounded inFlight set; every remaining "tombstone" mention is a comment, guarded by a test). The k8u flood test cancels an IN-FLIGHT invocation and hammers 300 cancel/resume cycles: the late-settling result can never post (exactly one typed cancellation), and a post-flood invoke still succeeds; removing the settlement-time epoch fence turns the test RED.
