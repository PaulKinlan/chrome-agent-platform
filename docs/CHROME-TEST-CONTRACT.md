...
- **`npm run test:changed`**: Resolves static reverse dependencies of changed files (against the **merge-base** with `origin/main`, not the moving tip). If `tests/chrome-profile-location.test.ts` or its dependencies (`scripts/lib/chrome-profile-dir.ts`, `scripts/lib/chrome-launch.ts`) are changed, it executes and launches real Chrome. If a change cannot be proven covered, `test:changed` fails closed to the full suite and thus launches real Chrome.
  - **chrome-agent-platform-nco2:** two file classes that have no import edges *by construction* are now mapped to the tests that cover them instead of forcing the full suite — an unimported `scripts/**` harness maps to the tree-walking guards (which enumerate `scripts/` at runtime), and a **version-only** bookkeeping-JSON diff maps to the tests that read those files as data. A bookkeeping JSON with any non-version change (dependency, script, permission, CSP) still fails closed, decided by parsing both sides rather than filtering lines containing `"version"`. **Every fail-closed run now names the file and the mechanism**, so a full-suite fallback says why it ran. Mapping a harness does NOT mean it is executed: no unit test spawns those harnesses, so the guards are the coverage the full suite would also have provided.
