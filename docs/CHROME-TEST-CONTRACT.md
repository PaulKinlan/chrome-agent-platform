...
- **`npm run test:changed`**: Resolves static reverse dependencies of changed files (against the **merge-base** with `origin/main`, not the moving tip). If `tests/chrome-profile-location.test.ts` or its dependencies (`scripts/lib/chrome-profile-dir.ts`, `scripts/lib/chrome-launch.ts`) are changed, it executes and launches real Chrome. If a change cannot be proven covered, `test:changed` fails closed to the full suite and thus launches real Chrome.
  ...
  - **The guard mapping is bounded by what the guards actually enumerate.** `harnessFiles()` scans `scripts/*.ts` only, so a `.mjs` helper or a subdirectory file is NOT covered by those guards; it maps only to tests that NAME it, and fails closed when none do. **A test can EXECUTE a script with no import edge** — `tests/acp-service-harness-default.test.ts` builds `join(ROOT, "scripts", "acp-service.mjs")` and spawns it — which is why literal references count as coverage and why an earlier claim here that "no unit test spawns those harnesses" was wrong and is retracted.
