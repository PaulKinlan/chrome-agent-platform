...
| **Resolved** (2026-09-11) | `README.md`, `PLAN.md`, `docs/CONSTITUTION.md` | **Repository guidance reconciled with shipped permission reality.** The manifest declares install-granted `host_permissions: ["<all_urls>"]` and install-granted `storage`, `alarms`, `contextMenus`, `unlimitedStorage`; all browser capability permissions (tabs, scripting, bookmarks, etc.) are optional on demand. | Core docs reconciled to state install-granted vs optional permissions truthfully. | P1 | Documentation review; manifest/permission policy tests |
