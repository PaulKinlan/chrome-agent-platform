...
- JXL decode SHIPPED AS jxl (2026-09-07, chrome-agent-platform-agpu, option B — the y75s/compressops shape): jSquash/jxl is Emscripten-glue blocked in d12t behind ltkj.3. The shipped tool is a CAP-authored WASI preview-1 driver over the jxl-oxide 0.12.6 crate and png 0.17.16 crate (pure Rust, no C dependencies, no rayon), base64 JXL on stdin → raw PNG bytes on stdout, 1,376,194 bytes, 8 wasi_snapshot_preview1 imports, default tier (2048 pages), sha256 d268e1ced9db8192d986d2138b03dcec174a88865e10e9f9c7c8e53ab3c0010a, byte-reproducible (build-a == build-b). Measured against Chrome's native APIs: createImageBitmap and <img> decode reject JXL on Chrome 150; jxl decodes 1 MP in ~290 ms (506 pages) inside the 5 s wall bound. Evidence: packages/bundled/evidence/jxl/, cap-evidence/cap-jxl/.
- AVIF encode SHIPPED AS avif (2026-09-07, chrome-agent-platform-ou4x, option B): Chrome's canvas cannot encode AVIF (convertToBlob({type:image/avif}) returns PNG bytes — cap-evidence/cap-avif/native-probe.md). The shipped tool is a CAP-authored WASI preview-1 driver over ravif 0.13.0 + rav1e 0.8.1 (BOTH default-features = false — no nasm/cc SIMD asm, no rayon threading; the software path), base64 image text on stdin → raw AVIF bytes on stdout (the job lane's base64 stdin/stdout, 8oil). 1,436,513 bytes, default tier (min 21 / max 2048 pages), sha256 efafe563c9aa683d8688d17f477584c04f17ba4cac5a52d0df027bcd76e1e294, byte-reproducible (build-a == build-b). Measured fit (cap-evidence/cap-avif/fit-report.md): 512×512 speed-10 ≈ 278 ms, worst 2,886 ms (1024×768 speed 8) — inside the executor's 5 s wall; that fit was the admission gate. rav1e's software path needs WASI sched_yield (parking_lot); the executor's shim implements it as an honest no-op (the m3vb clock_time_get precedent). Evidence: packages/bundled/evidence/avif/, cap-evidence/cap-avif/.
...
- Shipped instead: **compressops** (cap.bundled.compressops 1.0.0) — CAP-authored clean-room WASI preview-1 tool over the pure-Rust `zstd` (MIT OR Apache-2.0) and `brotli` (MIT OR Apache-2.0) crates: zstd and brotli compression/decompression + info frame inspector, stdin→stdout, 1,411,911 bytes, 8 WASI imports, sha256 3eb5e7391eefe588758169d012186064577c9e9060af8e027c33702e2aa207ce, byte-reproducible (build-a == build-b). Evidence: packages/bundled/evidence/compressops/. **Job-lane binary I/O (8oil, 2026-09-07):** the compression subcommands write frame BYTES and `-d`/`info` read a frame — the job lane now carries bytes end-to-end through gzip's generalised mechanism (`previewStdinEncoding`, the mirror of `previewStdoutEncoding`): compress text (stdin) → base64 frame, decompress a base64 frame → base64, info reads a base64 frame → the JSON report (utf8). Previously its compression was never executable in a live run (utf8-rejected).
