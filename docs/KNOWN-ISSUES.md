...
- **System-prompts surface (feat/system-prompt-settings, correction landed, awaiting independent re-review):** the layered/versioned system-prompt architecture + Settings → Advanced UI. The round-1 review BLOCKED the first commit; the correction landed: lib/runtime-policy.js as the single authoritative protected-constraints source (generated, drift-tested, composed LAST after skills), the run-bound provider-boundary attestation (exact wire digest + runId, journaled), the strict-schema/quarantine + revision-CAS + key-authority + agent-lifecycle persistence, UTF-8 byte bounds + malformed-Unicode rejection, keyed (HMAC) receipts replacing public FNV fingerprints, the storage-permission-on-Save durability flow, the inherited-upgrade keep/reset fix, and the dirty-scope confirm. 37 unit tests + a 42-check real-extension journey with real pointer/keyboard input. The standing independent review (a different model) must confirm the diff + evidence before this is considered cleared.
