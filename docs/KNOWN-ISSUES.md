...
## Review process
- 30 rounds of independent security/correctness review against the Constitution.
- Last retained clean baseline: **119/119 CDP journeys at `cd4887a`**. Current source/unit/browser results must be read from the exact-commit external evidence for the corrective commit; historical totals are not presented as current proof. All permissions remain optional (`manifest.permissions = []`) and no debugger permission is declared.
