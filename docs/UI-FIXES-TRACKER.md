...
4. **Structured tool-call renderer** — ~~Paul flagged live tool calls in a running job show as ugly raw JSON (escaping/encoding artifacts) instead of a usable structured view. Reproduce in the ACTUAL loaded MV3 extension → implement a polished renderer~~ → **FIXED (2026-08-18)**. Commits `7ab58a9` → `ec29dd2` → `c3ea53a` → `5a5567b` → `7d992d0` → `f378692` on `fix/tool-tree-renderer` (worktree `/tmp/tool-tree-wt`).
   ...
   - **Tests:** `tests/tool-tree.test.ts` (31) — incl. the atomic hostile ARRAY-PROXY test, the UTF-8 byte-cap envelope test (every maxBytes: valid JSON ≤ the cap, quote/backslash/non-ASCII), secret redaction, cycle-vs-alias. `tests/tool-lifecycle.test.ts` (18) — incl. two same-name calls with distinct original callIds, correct duplicate semantics (a normal pair is NOT a duplicate; a second same-type row IS), terminal-row args restore. `tests/run-terminal.test.ts` (8) + `tests/agent-abort.test.ts` (3) — a REAL createAgent run aborted MID-RUN via the production abort path: the DURABLE per-run flag is observable after orch.run resolves (the SW returns {ok:false, aborted:true}, never a success). Targeted 60 (31 + 18 + 8 + 3). Full `deno test -A tests/` 381 passed, 0 failed; `test:security` 7/7. — incl. the serializer's atomic build-then-emit (a hostile array-proxy length getter can never emit invalid JSON), UTF-8 byte bounds, secret redaction. `tests/tool-lifecycle.test.ts` (15). `tests/run-terminal.test.ts` (8) — the DETERMINISTIC no-grace ordering boundary tests (port-before-response, the ABORTED response never appends a success result/reports done, an aborted port done gates the outcome, late events DROPPED after settle, {ok:false} settles immediately, port DISCONNECT settles fail-closed + removes the listener, RETRY gets a UNIQUE runId — stale events rejected, exactly-once with no timers). Targeted 52. Full `deno test -A tests/` 372 passed, 0 failed; `test:security` 7/7. — nested objects/arrays, escaped JSON strings, Unicode (CJK), malformed JSON, huge/deep payload bounds, null/booleans/numbers, tool-error shapes, adversarial input, double-encode, subtree copy, dotted/empty/numeric-looking keys, `__proto__`-as-data, cyclic/getter/BigInt safety, `safeJsonStringify`, the serializer's NEVER-THROWS contract (hostile toJSON+toString), cycle-vs-alias, bounds. `tests/tool-lifecycle.test.ts` (15) — the FIFO queue, flush-on-done/error/abort, error-aware status wiring (`ok` AUTHORITATIVE — heuristics only when absent), SW-side `isToolResultFailure`, `pairToolJournal` (pairing, failed→error, FIFO, unpaired→done, passthrough, LEGACY no-callId pairing, absent-ok text heuristics, run-instance isolation, unmatched-result uniqueness). `tests/run-terminal.test.ts` (5) — the DETERMINISTIC ordering boundary tests at runConversationTurn (port-before-response; response-before-port with a delayed aborted-done / delayed ok:false result; {ok:false} with no port final; exactly-once arbitration). Targeted total 48. Full `deno test -A tests/` 369 passed, 0 failed; `test:security` 7/7.
   - **sol-review corrective commit `5a5567b`:** terminal ARBITRATION (a grace window — the listener stays subscribed so a delayed tool-result ok:false or done aborted:true WINS over a response that raced ahead; the queue settles exactly once; both orderings tested deterministically at the runConversationTurn boundary); persisted replay (run-instance-scoped callIds + persisted ok + unique orphan-result ids + legacy (id,tool,occurrence) pairing + absent-ok text heuristics + the component maps "done" to a TERMINAL card, never running); the public serializer is a MANUAL bounded one (cycle-vs-alias correct, depth/node/byte/string caps enforced while serializing, a valid-JSON `__gvs_truncated__` envelope, NEVER throws); copy edges (empty strings copy, truncated subtree JSON stays VALID with explicit metadata, execCommand(false) reports failure); register-task gains scriptId route parity.
   - **Real-extension evidence:** `scripts/tool-call-evidence.ts` loads the ACTUAL MV3 extension + the real NTP; a REAL demo-provider run opens the thread view + renders live bubbles; the EXACT live-path calls (`appendTool` + status/result/duration attribute updates) drive a real memory_set card. RAW mode (pre-fix code) captured the escaped-JSON bug (8/8); TREE mode (37/37) asserts the structured tree, expansion/collapse, bounds, no-innerHTML, timing, nested keys, no-phantom-duration, error-status rendering, REAL copy clicks (leaf VALUE + bounded subtree JSON + a REJECTED write does NOT claim "copied" + an EMPTY-string leaf copies + a truncated subtree copy stays VALID JSON with `__gvs_truncated__`), cyclic AND hostile (toJSON+toString) args safety at the public appendTool boundary, request finalization (zero running cards after a real run), persisted-replay pairing (one terminal card per call; ok:false → error; an unpaired/missing-result card renders TERMINAL "done"), and REAL persistence → RELOAD → reopen (the demo run's journal rows survived a page reload + the task thread reopened; read back via memory.get). Screenshots + DOM evidence in `test-artifacts/tool-call/`.
