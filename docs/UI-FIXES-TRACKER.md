...
- **Fix:** the picker commits typed text on blur and via a new `commitTyped()` drive hook (the Use handler calls it before reading the value); `providerRunGate` + `provider.set` refuse `model id missing` for a provider that needs a model and has neither an explicit id nor a catalogue default (keeping the previous model on refuse); `resolveModelFromConfig` THROWS instead of falling back to the demo for a real provider id — the hub shows the red Settings remediation chip instead of a silent demo answer.
- **Evidence:** unit (typed-text commit on blur/commitTyped/arrow-guard; gate refusals; refusal surfaced through the save binding) RED→GREEN falsification; journeys "Settings: typing a model id and clicking Use saves it" + "Settings: saving with an empty model shows a red status" (screenshots `settings-byo-typed-model.png`, `settings-byo-empty-model-red.png`).
