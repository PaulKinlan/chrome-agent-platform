...
### Task-view View Transition ghost (2026-08-21 — provisional current-main 0.2.116 reconciliation)
...
- Reviewed successor `0d3199a` is reconciled by content onto Directory main `eed40358`. It preserves direction policy and shared `overlay-view` identity. Under the explicit task-boundary class, both old `root` and old `overlay-view` are transparent/non-animated; new `overlay-view` is not targeted and remains named/active. Complete task enter/exit route tests cover Hub, Settings, Directory, Skills, and Artifacts; unrelated full-view routes retain normal named cross-fades. Cleanup/focus/race/reduced-motion and fail-closed changelog shipping remain intact. On same-surface task→named-agent switches, `showThreadView`'s already-open branch synchronously routes focus to the thread composer with a connected/visible focus guard, preserving no-flash and no-transition invariants. Directory's exact covered `side` + `sideToggle` inert/`aria-hidden` behavior and initiating-trigger focus restoration are retained and composed with post-transition focus.
- Still open: independent current-main source/a11y review and corrected loaded-MV3 midpoint captures for launch, task→Settings, and keyboard restore, with zero errors and post-finish focus. Immutable v2 stopped later on a harness syntax error, so keyboard/pointer checks are unverified. The corrected harness must accept `durable-run-registry` as valid document-level Shadow DOM retargeting, dispatch a genuine pointer, and prove View logs opens; no product workaround is required. Reconcile provisional 0.2.116 with concurrent candidates before integration.
