...
4. **Structured tool-call renderer** — ~~Paul flagged live tool calls in a running job show as ugly raw JSON (escaping/encoding artifacts) instead of a usable structured view. Reproduce in the ACTUAL loaded MV3 extension → implement a polished renderer~~ → **FIXED (2026-08-18)**. Commits `7ab58a9` → `ec29dd2` → `c3ea53a` → `5a5567b` → `7d992d0` → `f378692` → `206bf95` → `4285acd` → `e74d294` → `991538b` → `c673886` → `6315311` on `fix/tool-tree-renderer` (worktree `/tmp/tool-tree-wt`).
