...
- **A fresh profile listed 22 disabled templates as agents in the hub sidebar and the side panel (2026-08-30, CAP-FB-20260830-FRESH-PROFILE-TEMPLATE-AGENTS-01)** — the sidebar was fed a second "navigation" projection that re-admitted every disabled recipe, so the first screen showed "Sorting Hat … Schedule off" 22 times beside a panel saying "0 agents". The hub sidebar, the hub Agents panel, the side panel Agents tab and Settings → Agents now share ONE projection: created named agents plus enabled background agents; a fresh profile shows the empty state in all four. The recipes are not cut — they stay reachable as templates through the create dialog and Settings' "Configure" picker. The journey suite asserts the four counts agree at 0, at 1 after enabling one recipe, and at 0 again.
...
### Typed-but-unpicked model saves model:"" and runs the demo (Paul 2026-08-30 — FIXED, CAP-FB-20260830-MODEL-FIELD-EMPTY-SAVE-01)
- Typing a model id into `<model-picker>` without clicking a suggestion saved `model:""`, and a no-catalogue provider (BYO / Ollama / LM Studio) then silently ran the DEMO model — the owner typed a real model and got "[demo model] Task received".
- **Fix:** the picker commits typed text on blur AND via `commitTyped()`, which the save path (`providerFieldsFromCard`) calls before reading the value — so a Use click that never blurs still saves the typed id; `providerRunGate` + `provider.set` refuse `model id missing` for a provider that needs a model and has neither an explicit id nor a catalogue default (keeping the previous model on refuse; local providers exempt); `resolveModelFromConfig` THROWS instead of falling back to the demo for a real provider id; the refusal is settled as a CONFIG error (no provider retry/breaker noise) and the Settings flash reports it instead of "Set … as default.".
- **Evidence:** unit (typed-text commit on blur/commitTyped/arrow-guard, the no-blur Use-path commit, gate refusals, refusal surfaced through the save binding, model-missing never classified as a provider error) RED→GREEN falsification; journeys "Settings: typing a model id and clicking Use saves it" + the empty-model save refusal + the red hub chip for a broken stored config (screenshots `settings-byo-typed-model.png`, `settings-byo-empty-model-red.png`). Scope note: the model-missing hub-chip state is unreachable by design once `provider.set` refuses empty-model saves — the hub red-chip check therefore uses a broken stored endpoint config, which is the honest persistent red state.
...
- **Owner correction (2026-08-29, candidate in review):** **Find site tools** must never list every web tab. `CAP-FB-20260829-WEBMCP-PASSIVE-DETECTION-01` passively detects WebMCP before enrollment, stores only recent positive origins in a bounded registry, and feeds that same eligibility rule to every picker/proactive surface. The browser journey is 36/36: the WebMCP fixture appears before enrollment and a concurrently-open plain page does not.
- **Residual:** independent review of the candidate is required before integration.
...
### Conversation / thread
- **Thread view run state, identity and layout (2026-08-30, CAP-FB-20260830-THREAD-VIEW-RUN-STATE-01 — candidate):** the conversation is content-height inside a scrolling thread body with the composer docked (`position: sticky`) at the viewport bottom — no 620px bordered box with 440px of empty panel under two bubbles. Assistant turns carry `<agent-identity>` (avatar · name · `<time datetime>`). The run row reads "Working — reading your tabs…" from the progress port, hosts the shared `<loading-state>` grid with elapsed seconds, and hides on completion. Appends stick to the bottom unless the owner scrolled up (24px slack); the owner's send re-sticks. A generated-page card is titled with the artifact's name, never "Generated UI". The programmatic title focus paints no ring. Four loaded-extension journeys at 1440x900 (+ a 1024x700 screenshot) cover it.
- **macOS dictation diagnostics (2026-08-29, CAP-FB-20260829-MIC-DEAD-MACOS-01 — candidate):** dictation no longer awaits the decorative `getUserMedia` meter. With at least two physical audio inputs, a small CSS-anchor-positioned popover enumerates them, requests labels once, persists a meter-only selection, and shows a brief genuine level check on that exact device. The popover and no-speech/audio-capture errors say plainly that Web Speech cannot be retargeted and follows the OS default input, with the macOS System Settings path. Device disconnects fall back honestly; rejected meter capture labels the waveform as a non-live recording animation. The real-browser KAT covers pre-permission default-only enumeration followed by post-grant discovery, the device-count threshold, persistence, exact-device constraints, out-of-order reselection streams, devicechange, diagnostics, fallback honesty, and all prior lifecycle races.
...
- **Owner-grade console and run logs (2026-08-29 — in review)** — Advanced Settings exposes Off/Normal/Verbose and an explicit “Full detail in local logs” switch. Verbose emits paired tool-call start/end entries with arguments, resolved source/name, outcome/error and duration; full detail affects local DevTools only, while the trace/export ring remains redacted. Task and agent headers keep a visible Run logs affordance after settlement; the retained registry pages every run and each row loads a bounded latest-200 timeline.
