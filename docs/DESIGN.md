...
- Permissions is a grouped list (Browsing · Content · System · Always on): one native `<details>` per group with an "N of M on" count readable while closed; a group opens by default only when something in it is on, and the owner's open/closed choice survives a re-render. Every row is the shared `<capability-row>` — a state glyph in the icon column (accent only when on), a ghost **Turn on** button whose click is the permission gesture, the shared `<switch-toggle>` once granted, plain text for fixed states, and the "Gates:" sentence behind a **What it allows** disclosure. Hooks (developer-only) is one `<table>` — Event · Chrome API · Allowed — with one `<tbody>` per chrome.* API behind an `aria-expanded` group row and a `<switch-toggle>` per event; the Chrome API column hides below a 720px container. The danger token appears only in the destructive confirmation dialogs (Turn off, Deny all), never on a row control.
...
- Saved rows show the full name and description, exact byte count, added time, and a native disclosure for the full SHA-256 digest. Owner text is inert text, never HTML. No list or description truncation.
- Remove uses the existing destructive confirmation dialog; errors remain visible and offer Refresh list. The same component appears in the gallery with explicitly illustrative records and no persistence backend. Storage behavior is documented in [USER-WASM-STORAGE.md](USER-WASM-STORAGE.md).
