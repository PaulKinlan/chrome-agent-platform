...
**2026-08-28 follow-on — the unified agent model (owner directive, landed with the template picker):** an agent is persona + skills + memory + an OPTIONAL schedule. One creation flow (the create dialog's schedule field), one agents list (schedule chip, no background segregation), one schedule code path (`named-agent.set-schedule` → `agent:<slug>` alarms → the fire path runs the agent's real persona/skills/memory). Background templates now just prefill the schedule field. ~~**Documented follow-on:** the two underlying stores (`namedAgents` records + background-mode skills) are NOT yet unified~~ **DONE (chrome-agent-platform-wz6i, 2026-09-26):** built-in background agents are now SEEDED named-agent records (`lib/agent-seeds.js`) — the agent store is the ONE record authority. A seed carries its skill, its default cadence, and its legacy runtime identity (`background:<id>` surface, `recipe:<id>` memory tier) so runs, journals and attribution are byte-identical; enable/disable rides the ONE `agent:<id>` schedule path, existing `recipe:<id>` schedules re-key at startup preserving the exact next-fire, and a disabled seed stays a template (never an agent row). Custom duplicated background skills keep the legacy path; the physical `recipe:` renames (OPFS dirs, hooks records) remain chrome-agent-platform-e5oe.
