...
| G5 | **Agent-to-agent delegation** (worker → reviewer loops) | implied | implied ("what would your manager say") | **partial** — hub→agent via `delegate_task`; no agent→agent edges | Allow `delegate_task` inside an agent run to target another named agent (depth-limited, loop-guarded). **M** |
...
3. **Agent→agent delegation with loop guards** (G5, M): allow `delegate_task` inside agent runs targeting named agents (depth ≤2, cycle-detect, per-edge grant still enforced by the SW authority). Unlocks Team Lead/Critic for real.
