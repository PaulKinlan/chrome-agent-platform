...
### [Catalog public; shadow lazy successor in review; provider cutover blocked] Co-do-style Wasm/tool OS — `CAP-FB-20260822-WASM-TOOL-PLATFORM-01`
- CAP has a public bounded metadata catalog, deterministic lexical index, and expiring run/agent/origin/document/catalog/source/package-generation selection references. A shadow-only lazy successor defines the fixed two-tool wire, selected-only capture and an injectable execution core that repeatedly re-resolves live authority around the existing source closures. Search creates no grant; references authorize nothing; missing/WebMCP replay metadata remains unknown; eager provider binding is unchanged.
- Provider exposure remains blocked on exact loaded-MV3 nondisclosure/uncallability/lifecycle evidence. The wider execution platform is still open: loaded-MV3 Wasm/Worker/offscreen/OPFS behavior, import and memory validation, package WAL/CAS, signer/revocation/SBOM/licence authority, fresh-Worker termination, quotas, artifact promotion, diff review, Chrome lazy tools, Tool Library, owner install, bundled tools, spreadsheets, and abuse gates.
