...
- **Security-suite custody (fixed locally; independent review pending):** `npm run test:security` now acquires the canonical lock before side effects and supervises the fixed real runner under an immutable 120-second deadline, exact wrapper profile, verified PID=PGID=SID group, bounded TERM→KILL, guarded cleanup, durable evidence and escaped-descendant poison. Direct/no-lock/stale/timeout/signal/symlink/ownership/residue paths use executable no-Chrome mutants against shared live helpers; no Chrome acceptance claim is made before review and coordinator authorization.
