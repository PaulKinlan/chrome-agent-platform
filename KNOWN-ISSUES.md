...
- **Package archive freshness (public fix; deterministic-marker successor in review):** public `1fd65c696cbfcbe0aed135e0ba8c743b8c0ca624` derives an exact tracked-plus-generated regular-file inventory, verifies a fresh same-directory temp ZIP by names/hashes/extraction, and atomically replaces the final. A later residual showed `dist.complete` still embedded a random lock owner and wall-clock timestamp. The narrow successor keeps that custody private and writes canonical commit/indexed-source/output-hash marker bytes, with pre/post-inventory validation and hostile stale/owner/timestamp mutants. Same-source two-build markers and ZIPs are byte-identical; ignored artifacts and removed files still cannot survive. Independent successor review remains pending.
