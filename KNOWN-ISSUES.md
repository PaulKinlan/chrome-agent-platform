...
- The provisional current-main 0.2.116 reconciliation retains the source/target task-boundary policy and named `overlay-view` identity. During either task entry or exit it hides old `root` and old `overlay-view` pixels; it does not target new `overlay-view`, so the replacement named transition remains active. Hub→Settings and all other unrelated full-view routes retain their normal named cross-fade. Finish/abort/overlap cleanup, post-settlement focus, and reduced-motion fallback remain covered. Same-surface task→named-agent switches route focus to the thread composer synchronously when the thread view is already open, preserving no-transition and no-flash invariants. Directory's full-view authority is composed rather than replaced: both the sidebar and edge control remain inert/`aria-hidden` while covered, and the initiating trigger still receives deferred focus restoration after close.
...
- The immutable v2/v3/v4 browser evidence confirmed zero old task/named-overlay pixels at 40/125/220ms and verified genuine keyboard picker commit and pointer send; v4 isolated dropped composer focus on same-surface agent switches to `showThreadView`'s already-open branch, which is now fixed. Future evidence must run the corrected focused loaded-MV3 journey verifying midpoint zero-ghost policy, keyboard picker selection, pointer send, composer focus retention, and singular run/thread projection. Independent review of this current-main conflict disposition and loaded-MV3 midpoint/error/focus/pointer evidence remain required. The 0.2.116 release identity is explicitly provisional until serialized integration.
