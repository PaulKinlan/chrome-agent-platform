...
- **Package archive freshness (fixed locally; independent review pending):** the prior package command copied the whole local extension tree and let `zip` update an existing final archive, so ignored artifacts or removed files could survive. `CAP-FB-20260822-PACKAGE-ARCHIVE-FRESHNESS-01` now derives an exact tracked-plus-generated regular-file inventory, verifies a fresh same-directory temp ZIP by names/hashes/extraction, and atomically replaces the final; poison/removal/symlink/special/failure-cleanup regressions are green.
