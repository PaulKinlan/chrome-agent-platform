...
const wrap = code => new Map([["background/service-worker.js", code], ["options.bundle.js", "export const options=1;"], ["workers/agent-worker.js", "export const worker=1;"], ["shared/diff-core.bundle.js", "export const diff=1;"]]);
...
Deno.test("validate origin: actual esbuild canonical positive, tree-shaken presence and foreign substitution refuse", async () => {
  try {
    ...
    let clean, originError = null;
    try { clean = builder.finish(wrap(built.code)); } catch (e) { originError = e.message; }
    assertEquals(originError, null, "j6au keeps validation in SW, not the static Options rehydration host");
    ...
    assertEquals(wasmCallSites(clean.outputs.get("background/service-worker.js"))[0].arguments.length, 1);
    assertEquals(wasmCallSites(clean.outputs.get("options.bundle.js")), []);
    assertEquals(clean.derivation.sites.map(s => s.output), ["background/service-worker.js"]);
    ...
    for (const forbidden of [foreign, built.code]) {
      const optionsCall = wrap(built.code); optionsCall.set("options.bundle.js", forbidden);
      assertThrows(() => builder.finish(optionsCall), Error, undefined, "Options has no marked or unmarked Wasm allowance");
    }
...
Deno.test("validate origin: strict marker/physical-logical policy accepts bound output and rejects missing/stale/mismatched derivations", async () => {
  ...
  try {
    ...
    const copied = path.join(dir, "copied.js"); await writeFile(copied, clean.outputs.get("background/service-worker.js"));
    const wrongPhysical = inventory.map(e => e.archivePath === "dist/background/service-worker.js" ? { ...e, sourcePath: copied } : e);
