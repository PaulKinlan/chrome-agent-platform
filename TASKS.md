...
| P1 | IN_REVIEW | [`CAP-FB-20260828-MIC-RECORDING-STATE-01`](#cap-fb-20260828-mic-recording-state-01-the-mic-showed-no-recording-state--and-its-start-path-leaked-streams-and-contexts) | The mic showed no recording state — and its start path leaked streams and contexts |
...
## [CAP-FB-20260828-MIC-RECORDING-STATE-01] The mic showed no recording state — and its start path leaked streams and contexts
- Feedback: 2026-08-28 — product owner UX bug wave: the composer's mic button gives no sign it is recording, dictation can keep running in the background, and send-while-dictating leaves the mic open
- Updated: 2026-08-29 00:30 UTC
- Status: IN_REVIEW
- Resume: —
- Priority: P1
- Owner: coordinator worker lane
- Workspace: active (local path private)
- Branch: `cap-mic-state`
- Base: `c9635ab5`
- Candidate: this tracker commit (round 2); round 1 landed as `ce6247ef`
- Shipping: —
- Acceptance: recording is unmistakable (accent + live AnalyserNode waveform or honest CSS fallback, hover STOP affordance, aria-pressed/aria-label track state); a start whose getUserMedia is still PENDING is cancelled by send/stop, disconnect, view-hide, or a second click — the late stream is stopped, never adopted; detach/reattach never resurrects a false recording affordance from the stale attribute; a failed AnalyserNode setup closes the half-built AudioContext; onend restart-storms revert to idle honestly
- Review: independent review by a different model/session; round-1 review PASSed the visuals and returned the four P1s this round fixes
- Gates: full unit suite; `tests/mic-button-state.test.ts` falsification-gated (7 behavioral RED on a clean `ce6247ef` checkout + 1 control pass, GREEN after the fixes); real-browser `scripts/kat-mic-state.ts` 40/40 with the detach/reattach + pending-cancel checks shown RED against the pre-fix build; `npm run build:production`
- Blockers: —
- Next: independent review of the round-2 candidate, then merge
- Recover: `git grep -n "_startGen" -- extension/shared/components.js && git grep -n "_requestMicStream" -- extension/shared/components.js`
- History:
  - 2026-08-29 — **round 3 fixed; RED evidence regenerated honestly.** Two P1s from round-2 review. (a) PENDING pagehide not invalidated: `_stopIfHidden()` and the `pagehide` handler both no-oped while `_listening` was false (exactly the pending-getUserMedia state), leaving the start's generation valid — a late stream could be adopted after the page hid. Both now bump `_startGen` UNCONDITIONALLY before the listening check; new test dispatches a real `pagehide` mid-prompt and asserts the late stream is stopped and never adopted. (b) The round-2 unit RED log was not genuine (0 passed / all cancelled on an `_ensureMicPermission`→`_requestMicStream` rename mismatch): regenerated against a clean `ce6247ef` scratch checkout — 7 behavioral AssertionError failures + 1 control pass (the happy-path test correctly stays GREEN pre-fix); replaced at `~/.local/state/chrome-agent-platform/mic-r2-evidence/mic-r2-unit-RED.log`.
  - 2026-08-29 00:30 UTC — **round 2 fixed; red-then-green proven.** Four P1s from round-1 review. (a) PENDING getUserMedia race/leak: `start()` now runs under a generation counter (`_startGen`) bumped by every start, `stop()`, and `disconnectedCallback()`; a late-resolving stream whose generation was superseded is stopped and never adopted, and a composer that went display:none mid-request gets no background recording (owner told via mic-error). (b) disconnect now removes the host `listening` attribute AFTER `super.disconnectedCallback()` sets `_rendered=false`, so the removal cannot re-render the detached element and a reattach renders idle. (c) `_startMeter()` closes the half-built AudioContext when `createAnalyser`/`createMediaStreamSource` throws before falling back to the CSS animation. (d) this tracker entry. Evidence logs (unit RED/GREEN, KAT RED/GREEN): `~/.local/state/chrome-agent-platform/mic-r2-evidence/`. Gates: unit 7/7 new + full suite 1933/0, KAT 40/40 (3 targeted REDs on the pre-fix build showing the shipped symptoms: stale `aria-pressed=true` on reattach, late pending start entering recording with a live leaked stream), build rc=0.
  - 2026-08-28 — round 1 (`ce6247ef`): `data-listening` was never rendered though the CSS keyed on it, so recording looked identical to idle. Fixed with the three-channel state signal (host attribute authority, data-listening visuals, aria-pressed), the real-level waveform with CSS fallback, hover STOP, send/view-switch/pagehide/onend-storm lifecycle. 32-check KAT falsification-proven against the pre-fix tree (8 FAILs including the shipped symptom).
