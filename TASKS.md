...
- Candidate: —
- Shipping: —
- Acceptance: every custom element defined in `extension/shared/components.js` that no extension page mounts has a recorded verdict — ADOPT (named page and owning entry) or DELETE (with the reason) — in the table below; the DELETE set is removed with its gallery specimens and the drift check stays green; the ADOPT set is left in place and each owning entry cites it; a `scripts/check-components.mjs` check runs in `npm run build` and fails when a `customElements.define` name is used by no non-gallery page and is not in an allowlist naming its owning entry. The owner asked "what are the gallery components? you didn't give details" — the table below is that answer. `theme-picker` is deleted first (its feature was removed at `0.2.301`).
  - Context: the reanalysis ui lane (finding 14) found fourteen elements defined in `extension/shared/components.js` and referenced by nothing under `extension/` except the define line (verified in this tree by grep over `extension/`, `scripts/`, `tests/` excluding `dist/` and `components.js` itself; the only hit outside is a comment at `extension/options/options.js:52`). `conversation-run-status` is a fifteenth in the same state and is listed because CAP-FB-20260830-THREAD-VIEW-RUN-STATE-01 adopts it. 42 elements are defined in total (8,855 lines). Owner directives 2026-08-30: agent templates are being INTEGRATED (`agent-template-card` is ADOPT, not delete); components the templates or the jobs board use are never cut. The install seed still writes `theme: "dark"` (`extension/background/service-worker.js:7724`) and `docs/DESIGN.md:27` still documents four themes; the gallery mounts `<theme-picker>` at `docs/components.html:16` and `:61`. Per-component table (name · what it renders · lines in `extension/shared/components.js` · verdict · target):
    - `agent-config-form` · a three-field form (Name, Instructions, Skills comma-list) with a "Save agent" button emitting `save` · `:6243-6275` · DELETE · superseded by the create/edit dialog (`openAgentDialog` in `extension/ntp/ntp.js`, the `agent-dialog` element); no page or test mounts it (verified).
    - `agent-nav` · a `role="tab"` strip over a `VIEWS` list with `aria-selected` and a `navigate` event · `:6634-6655` · DELETE · the hub navigates with the sidebar footer buttons and `openView`; no page or test mounts it (verified).
    ...
    - `prompt-bar` · a textarea with model pill, mic and attach buttons and a suggestion listbox · `:5650-5725` · DELETE · superseded by `agent-composer` (`:4157-5198`), which is the shipped composer; no page or test mounts it (verified).
    - `run-task-button` · an accent button with a spinner and `run-task` event · `:791-820` · DELETE · the composer's `send-label="Run task"` is the product's run control; five gallery specimens only; no page or test mounts it (verified).
    - `screenshot-strip` · a horizontal strip of 96x64 thumbnails emitting `open` · `:4117-4144` · DELETE · no page or test mounts it (verified); if CAP-FB-20260830-SCREENSHOT-TO-MODEL-01 wants a thumbnail strip in the thread it can restore it from history.
    ...
    - `theme-picker` · four 44 px swatches (Sunlit, Midnight, Neon, Terminal) emitting `theme-change` · `:1726-1753` · DELETE · the theme feature was removed at 0.2.301; also delete `docs/components.html:16,61`, the `theme` seed at `service-worker.js:7724`, and `docs/DESIGN.md:27`.
  ...
  - Files: `extension/shared/components.js` (delete `:791-820`, `:1726-1753`, `:4117-4144`, `:5650-5725`, `:6243-6275`, `:6634-6655` and the `THEMES`/`VIEWS` constants if nothing else uses them); `docs/components.html` (the matching specimens; `:16` and `:61` for theme-picker); `extension/background/service-worker.js:7724`; `docs/DESIGN.md:27`; new `scripts/check-components.mjs` (parse `customElements.define("name"` from `components.js`, grep non-gallery `extension/**/*.{html,js}` for `<name` or `createElement("name")`, compare against an `ALLOWLIST = { "agent-template-card": "CAP-FB-20260830-AGENT-TEMPLATES-INTEGRATION-01", … }`); `package.json` (`build` runs it; `npm run check:gallery` unchanged). Do NOT delete any ADOPT row; do NOT touch `agent-templates.js`, `agent-board.js` or anything the board sidebar (`extension/ntp/ntp.html:823`) renders.
  - Steps: (1) Write `scripts/check-components.mjs` with the ADOPT rows as the allowlist and run it — it must list exactly the six DELETE names as failures before any deletion (this is the falsification baseline). (2) Delete `theme-picker` and its three references (gallery, seed, DESIGN.md). (3) Delete `agent-nav`, `prompt-bar`, `run-task-button`, `screenshot-strip`, `agent-config-form` and their specimens; `npm run check:gallery`; `npm run test:components`. (4) Wire the check into `build`. (5) When an ADOPT entry lands it removes its name from the allowlist in the same commit; the check keeps the allowlist from growing silently. (6) CHANGELOG; `docs/DESIGN.md` component inventory.
...
- Blockers: `tool-chips` and `thinking-trace` must NOT be deleted before `CAP-FB-20260827-TOOL-CALL-LEGIBILITY-01` decides their use; the other ADOPT rows are protected by their owning entries; the DELETE rows have no blocker
- Next: write `scripts/check-components.mjs`, observe it name the six DELETE elements, then delete `theme-picker` — it is unambiguous dead code for a removed feature
