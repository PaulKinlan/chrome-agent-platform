...
> completed entries live in `TASKS-DONE.md`. Active counts: **115 nonterminal**; every 2026-08-30 entry is written in the detailed hand-off format (owner directive 2026-08-30).
...
| `CAP-FB-20260830-RUN-SCRIPT-FETCH-APPROVAL-01` | `cap/run-script-fetch-approval` | 2026-08-30 16:30 UTC |
...
- Blockers: —
- Next: coordinator merge of `cap/run-script-fetch-approval` (two commits). Follow-ups noted: the `scripts/security-suite.ts` checks wait on `SUITE-HONESTY-01` (the suite does not load the extension yet); DNS rebinding of a listed public host is bounded by the allow-list, not resolved-address checks.
