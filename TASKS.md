...
> completed entries live in `TASKS-DONE.md`. Active counts: **60 nonterminal**; every 2026-08-30 entry is written in the detailed hand-off format (owner directive 2026-08-30).
...
| `CAP-FB-20260830-MODEL-CALL-ECONOMY-01` | `cap/model-call-economy` | 2026-09-02 05:41 UTC |
| `CAP-FB-20260902-KAT-AGENT-DELEGATION-RED-01` | `cap/kat-agent-delegation-red` | 2026-09-02 05:41 UTC |
| `CAP-FB-20260830-HUB-CHROME-POLISH-01` | `cap/hub-chrome-polish` | 2026-09-02 05:41 UTC |
| `CAP-FB-20260830-EXEC-DEMO-01` | `cap/exec-demo` | 2026-09-01 20:33 UTC |

## Open work queue

**This file holds only what is in progress or still to do — 60 entries.** Completed work is archived in [TASKS-DONE.md](TASKS-DONE.md) at triage; **merged is done** (Paul, 2026-08-28), so nothing sits in a terminal state here. Most urgent first (regenerated 2026-08-30). The entry itself is always the authority; where it disagrees with this table, the entry wins.
...
| P1 | OPEN | [`CAP-FB-20260902-KAT-AGENT-DELEGATION-RED-01`](#cap-fb-20260902-kat-agent-delegation-red-01-kat-agent-delegation-is-deterministically-red-since-the-run-budget-change) | kat-agent-delegation is deterministically red since the run-budget change |
...
- Review: author review 2026-09-02 — falsification gates cleared (RED/GREEN recorded below). No independent second model available (fleet retired).
- Gates: the falsification gates apply
  - Unit: `tests/privacy-statement.test.ts` (6 tests) — RED before the module existed (`TS2307 Cannot find module 'extension/lib/privacy-statement.js'`), GREEN after (`ok | 6 passed | 0 failed`). Falsification of the host gate: an unexported `https://api.example-new.test/v1` literal planted in `provider.js` → `FAILED | 5 passed | 1 failed — AssertionError: extension/lib/provider.js sends to api.example-new.test but OUTBOUND_HOSTS does not list it`; restored → `6 passed`. The reset defects the browser run found got three tests in `tests/factory-reset.test.ts` — RED on the pre-fix tree (`Values are not equal: no database survives the reset`; `a surviving database fails the reset closed`; `the reset route calls functions the worker never defines: invalidateOrchestrator`), GREEN after the fixes (`15 passed | 0 failed`).
  - Browser: `deno run -A scripts/chrome-journeys.ts` — the `factory-reset` group (own Chrome, fresh profile): "privacy page: renders the lists and the host-access sentence", "privacy page: names every outbound host and every storage class the code declares", "privacy page: linked from Settings → About", "factory reset: every storage class is populated before the reset", "factory reset: the Settings button raises the destructive confirm dialog", "factory reset: after the genuine confirm every storage class is empty", "factory reset: the hub shows the first-run guide again", "factory reset: the seeded markers are gone and the run and thread lists are empty" — all PASS, `350/350 passed`. Screenshots `privacy-page.png`, `settings-factory-reset-dialog.png`, `settings-factory-reset-after.png`. `deno run -A scripts/a11y-audit.ts` extended with seven `privacy:` checks — `RESULT: 38 passed, 0 failed, 2 expected-red` (both pre-existing, owned elsewhere).
  - Full suite: `npm run build && deno test -A tests/ && deno run -A scripts/chrome-journeys.ts` at this branch tip — build clean, `check:vocabulary` green (17 surfaces), `check:gallery` clean, `ok | 3086 passed (14 steps) | 0 failed (14m15s)`, chrome journeys `350/350 passed` (baseline 2457 / 138 at `fc2255be`; the suite has grown since). (An earlier pass, run while this lane's own browser held the serialized-Chrome lock, showed the known lock-bound `tests/security-suite-custody.test.ts` red; alone at this tip it is `ok | 9 passed | 0 failed (1m41s)`, and the final full pass above is green with it included.)
...
- Blockers: — (HOST-ACCESS-STORY-01 landed at `origin/main@33a522e6`, Q18 (a))
- Next: coordinator merge; optional follow-up (owner decision) — move the provider key to `chrome.storage.session` or encrypt it; the page now says plainly that it is stored unencrypted.
...
- History:
  ...
  - 2026-09-02 08:10 UTC — candidate on `cap/privacy-statement` (rebased base `791c1ac2`). Delivered: (1) `extension/privacy/privacy.html` + `privacy.js` + `privacy.css` — "What this extension sends and stores", an ordinary extension page (not web-accessible), linked from Settings → About → Privacy and from the README; (2) `extension/lib/privacy-statement.js` — the PURE statement builder; (3) `extension/lib/provider.js` exports `OUTBOUND_HOSTS`, DERIVED from `PROVIDER_CHOICES` (never hand-typed); (4) the `<privacy-statement>` component in `extension/shared/components.js` with a gallery specimen (`docs/components.html`), every row data → textContent; (5) the `privacy.statement` service-worker route (hosts + the live run-log policy from `run.list`); (6) the `factory-reset` group in `scripts/chrome-journeys.ts` and seven `privacy:` checks in `scripts/a11y-audit.ts`; (7) `scripts/check-vocabulary.mjs` scans the new page and the statement module. The statement, line by line, with the module each is read from: SENT — "your messages, the page text you share and any screenshot … go to the model you chose in Settings, together with your key; the choices that leave this computer: OpenAI (api.openai.com), Anthropic (api.anthropic.com), Google Gemini (generativelanguage.googleapis.com), DeepSeek (api.deepseek.com), or an address you typed yourself" (`lib/provider.js` `PROVIDER_CHOICES` baseURLs → `OUTBOUND_HOSTS`; `lib/models/openai-model.js` fetch; the screenshot lanes `acceptsImageToolResults`); "Ollama, LM Studio and Chrome's built-in model run on this computer" (`provider.js` `http://localhost:11434/v1`, `http://localhost:1234/v1`, `models/prompt-api-model.js`); "when you enter a key in Settings, that provider's list of models is fetched with it" (`lib/model-catalog.js` `fetchLiveModels`, called from `options/options.js:685`); "an MCP server you add in Settings receives the tool calls … with the headers you typed" (`lib/mcp-config.js` `cap:mcpServers`, `lib/mcp-client.js` `transport.headers`); "a site whose tools you allowed receives the tool calls an agent makes on that page, inside the page" (`content/content-script.js` + `main-world.js` bridge, `lib/enrollment.js`); "importing a skill fetches its files from the address you gave — for a GitHub repository, through api.github.com and raw.githubusercontent.com" (`lib/skill-import.js:145,161,208`, `fetchDirectSkill` :115; pinned by the unit test); "a script an agent writes can fetch only the web addresses you approved on its card, and sends no cookies or logins" (`background/service-worker.js` `"cap:fetch"` `credentials:"omit"`, `redirect:"manual"`, `lib/fetch-policy.js` `checkFetchPolicy`); "nothing else leaves this computer; no analytics, no usage reporting" (the literal-host sweep of `extension/**`: only the hosts above plus documentation links; `lib/python-runtime.js`'s `cdn.jsdelivr.net` is RETAINED-unreachable — `scripts/check-reachability.mjs`, `lib/python-tool.js:41` refuses). READ — the Q18 (a) sentence from `lib/privacy-statement.js` `HOST_ACCESS_SENTENCE`, pinned to `README.md:92-94`. STORED — one row per `FACTORY_RESET_STORAGE_CLASSES` (`lib/factory-reset.js:14`), rendered from that constant: `chrome.storage.local` "the model you chose and its key — kept as you typed it, not encrypted … the sites and servers you allowed, your agents, tasks, schedules and the rules you set in Settings" (`lib/kv.js` over `chrome.storage.local`; `provider.js` `setProviderConfig` → `kvSet({ providerConfig })` with `apiKey`; `lib/named-agents.js`, `lib/mcp-config.js`, `lib/hooks.js`); `chrome.storage.session` "short-lived working state" (`lib/perm-lease.js:17`, `service-worker.js:738`); `in-memory-session-kv` (`lib/kv.js` session Map); `origin-private-file-system` "each agent's memory, kept apart per site …; the run logs (full detail for the newest 50 runs per task and 500 overall, up to 32 MiB; older runs keep a one-line summary); artifacts; screenshots; models you downloaded" (`lib/memory.js` `ROOT = "memory"`, `lib/durable-runs.js` `RETENTION_DEFAULTS` {50, 500, 32 MiB} read live from `run.list.retentionPolicy`, `lib/artifacts.js`, `lib/memory.js:1660` screenshots, `factory-reset.js` header "models"); `indexed-db` "the folders and files you gave an agent access to — the access itself, not copies" (`lib/fs-grants.js` `cap_fs_grants`); `cache-storage`; `chrome.alarms` "the timers behind your schedules". KEY HANDLING — write-only key box (`options/options.js:796,1385` `type="password"`, "write-only"), error reports scrub keys (`lib/diagnostics.js:84-85`), the storage hook never sees the key or internal keys (`service-worker.js:8824` `INTERNAL_PREFIXES` includes `providerConfig`). WIPE — Settings → Data & memory per-agent, and Reset all data (`options/options.js` `#factory-reset-btn` → `system.factoryReset`). No beads: `grep -rn beads extension` → nothing. TWO PRODUCT DEFECTS the browser-driven reset found and fixed here (the reset had unit coverage only): (a) `service-worker.js` `"system.factoryReset"` called an undefined `invalidateOrchestrator()` since the reset shipped (`243c6094`), so the route wiped everything and then returned `ok:false` — Settings showed "Reset failed" and never returned to first run; it now calls the defined `invalidateAgent()`. (b) `executeFactoryReset` resolved a `deleteDatabase` on `onblocked` and the verification never counted IndexedDB, so `cap-usage` (the worker's cached connection) and `cap_fs_grants` (a Settings page's connection) survived every reset while the report said clean; the wipe now waits for the real completion (bounded, `idbBlockedWaitMs`), reports `idb_remnants_detected` fail-closed, and both owners close on `versionchange` (`lib/usage-store.js`, `lib/fs-grants.js`). (c) With those fixed the browser showed a third, subtler failure: the worker keeps running through the wipe, so an already-queued usage write reopened `cap-usage` moments after its delete succeeded, and the verification — running in the same microtask queue — read the stores before that landed. The wipe now settles briefly, then sweeps and re-verifies whatever the verification actually sees (bounded passes), so the report describes the profile rather than the moment. The reset's class list and all-or-nothing semantics are unchanged. Verified with a scratch probe against the loaded extension: before the fix `{"ok":false,"error":"factory_reset_failed: invalidateOrchestrator is not defined"}` with `indexedDbDatabases:["cap-usage","cap_fs_grants"]` remaining; after, `{"ok":true,"verified":true,…"indexedDbRemaining":0}` and every class empty. Key stored unencrypted: unchanged (out of scope), now stated on the page. Two guards owned by other entries moved with this change and are updated in the same commit: `tests/sw-route-modularization.test.ts`'s route baseline gains `privacy.statement` (193 → 194), and `tests/single-source-helpers.test.ts` required the settle to use the canonical `sleep` from `lib/pure.js` rather than an inline promise-wrapped `setTimeout`.
