...
- Blockers: —
- Next: coordinator merge of `cap/model-call-economy`; then (follow-ups, NOT done here) first-class the top tools to remove the `search_tools` round trip, and re-measure on gpt-5.6-sol / claude-sonnet-5 when those keys work (the Anthropic key on this machine is dead; the cache breakpoint is proven at the provider boundary by unit test, not live).
