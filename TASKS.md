...
- Gates: full unit suite; `tests/mic-button-state.test.ts` falsification-gated (6 RED on `ce6247ef`, GREEN after the fix); real-browser `scripts/kat-mic-state.ts` 40/40 with the detach/reattach + pending-cancel checks shown RED against the pre-fix build; `npm run build:production`
- Blockers: —
