...
> completed entries live in `TASKS-DONE.md`. Active counts: **118 nonterminal**; every 2026-08-30 entry is written in the detailed hand-off format (owner directive 2026-08-30).
...
- Review: pending
- Gates: the falsification gates apply
  - Unit: `deno test tests/agent-display.test.ts` (or the file covering `backgroundAgentsForDisplay`/`projectUnifiedAgents`) — add "sidebar projection: disabled background agents are excluded, enabled ones and named agents are kept" (revert step 1, expect RED, restore, GREEN).
  - Browser: `deno run -A scripts/chrome-journeys.ts` hub group — add "fresh profile: the four agent surfaces agree (0)" and "after enabling one recipe the four agent surfaces agree (1)"; screenshot `fresh-profile-sidebar.png` showing the empty Agents column and "0 agents".
  - Full suite: `npm run build && deno test tests/ && deno run -A scripts/chrome-journeys.ts` green at the tip (last recorded 2358/0 and 138/138; re-count).
...
- Blockers: —
- Next: the one-argument change at `ntp.js:737` plus the RED/GREEN unit assertion.
...
- Blockers: —
- Next: step 1 — add `extension/lib/model-catalog.js` + `tests/model-catalog.test.ts`, then step 2's gate so the 127 current hits are enumerated by the tool rather than by hand.
