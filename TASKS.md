...
> completed entries live in `TASKS-DONE.md`. Active counts: **112 nonterminal**; every 2026-08-30 entry is written in the detailed hand-off format (owner directive 2026-08-30).
...
| `CAP-FB-20260830-UNTRUSTED-CONTENT-FENCING-01` | `cap/untrusted-content-fencing` | 2026-08-30 14:24 UTC |
...
- Blockers: —
- Next: coordinator merge (union with the `cap/run-script-fetch-approval` lane: that lane owns `cap:fetch` in service-worker.js — the one-line hook `tagUntrusted(...)` from `extension/lib/untrusted-fence.js` is documented in the History line below)
