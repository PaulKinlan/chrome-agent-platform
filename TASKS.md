...
- Blockers: —
- Next: stage 3 (chunked log pages) is where the remaining win is, and it is GATED on proving the idempotency question in the design's section 5 — today a duplicate append is impossible because the row key IS the content hash of the idempotency key, and chunking replaces that with a bounded digest set. Prove that against `resumeAfterInterruption`/`activateResume` before touching storage, or fall back to a per-execution digest index
