...
const SKIP_AMBIENT = Deno.env.get("OVFM4_SIMULATE_FIXED_CSP") === "1";

const MODULE_SOURCE = `
const SKIP_AMBIENT = ${SKIP_AMBIENT};
export default async () => {
  ...
  if (!SKIP_AMBIENT) out.cases.xhrPlain = await wait((res) => {
  ...
  if (!SKIP_AMBIENT) out.cases.xhrCors = await wait((res) => {
    const x = new XMLHttpRequest();
  ...
  if (!SKIP_AMBIENT) out.cases.webSocket = await wait((res) => {
  ...
  // 6. sendBeacon
  if (!SKIP_AMBIENT) out.cases.beacon = await wait((res) => {
    try { res("queued:" + navigator.sendBeacon(B + "/probe-beacon", "x=1")); } catch (e) { res("err:" + String(e)); }
  });
  // 7. an image element
  if (!SKIP_AMBIENT) out.cases.image = await wait((res) => {
    const i = new Image();
  ...
  // 8. EventSource
  if (!SKIP_AMBIENT) out.cases.eventSource = await wait((res) => {
...
// The sandbox's storage TEACHING guards (script-sandbox.js:28-68): localStorage,
// sessionStorage, cookie, indexedDB, caches and OPFS are redefined to throw an
// error that teaches the durable-store route. Stage 4 requires them to remain
// active inside an IMPORTED module, so the probe is a module reached through the
// import map — and it carries a paired positive (Math.max) so a probe that simply
// always reports "threw" cannot pass.
...
async function runModuleCases() {
  ...
  try {
    ...
    return await cdp.eval(page.sessionId, `(async () => {
      ...
      // ovfm.3 regression: the SAME exact source run twice must mint cleanly again,
      // and the host must leave no iframe behind between runs.
...
// ── jfpx: the two seams whose MECHANISM the end-to-end cases could not see ──
// Both are exercised directly through the shipped libs, and each asserts the
// message that identifies the mechanism:
//   • fetch-policy: private/loopback refused by URL VALIDATION (its own words),
//     a public host allowed, and the unapproved case refused by the GATE — three
//     distinguishable outcomes where the old case accepted any refusal;
//   • script-sandbox-modules: the LIB digest verifier refuses a wrong digest
//     BEFORE minting (mint count stays 0) — the second of the three verifiers,
...
async function runSeamCases() {
  ...
  try {
    ...
    await new Promise((r) => setTimeout(r, 1500));
    return await cdp.eval(page.sessionId, `(async () => {
      const pol = await import(chrome.runtime.getURL("lib/fetch-policy.js"));
      const mods = await import(chrome.runtime.getURL("lib/script-sandbox-modules.js"));
      const out = {};
      out.hosts = {
        loopback: pol.isPrivateOrLoopbackHost("127.0.0.1"),
        metadata: pol.isPrivateOrLoopbackHost("169.254.169.254"),
        ipv6Loopback: pol.isPrivateOrLoopbackHost("::1"),
        public: pol.isPrivateOrLoopbackHost("example.com"),
      };
      out.privateUrl = pol.checkFetchPolicy("http://169.254.169.254/latest/meta-data/", null);
      out.publicAllowed = pol.checkFetchPolicy("https://example.com/", { hosts: ["example.com"] });
      out.gateRefusal = pol.checkFetchPolicy("https://example.com/", null);
      const bytes = new TextEncoder().encode("export const jfpx = 1;");
      let mints = 0;
      const createBlobUrl = () => { mints++; return "blob:jfpx/1"; };
      try {
        const r = mods.verifyAndPrepareJsModule({ name: "jfpx-mod", digest: "0".repeat(64), bytes, createBlobUrl });
        out.libWrongDigest = { resolved: true, value: r, mints };
      } catch (e) {
        out.libWrongDigest = { threw: String((e && e.message) || e).slice(0, 140), mints };
      }
      return out;
    })()`);
  } finally {
...
// These were briefly lost in a refactor of this file and restored deliberately:
// without them every arrival below is just "a server that logs what it sees".
...
// jfpx finding 2: in this context the POLICY GATE refuses first, so this case
// cannot claim URL validation. Renamed to what it proves; the validation itself
// is proven below by its own message, with the private/public pair.
check("BRIDGED fetch to a link-local metadata address is refused in an UNAPPROVED context (the policy gate)",
...
// Two states are legitimate, and they are NOT the same test:
//   (1) ambient egress exists (the tree as shipped) → every ambient path must
//       have arrived, and every request must have carried an opaque `Origin: null`;
//   (2) no ambient egress exists (a locked-down CSP, or a skipped probe) → the
//       correct assertion is that NOTHING ambient arrived, bracketed by liveness
//       controls so an apparatus failure cannot masquerade as a fixed policy.
// glm-flash-1 measured the second state against the first version of this KAT and
// it read as a failure: "the fix working looks like an empty wire log."
const AMBIENT = ["/probe-plain", "/probe-cors", "/probe-ws", "/probe-beacon", "/probe-es"];
const ambient = seen.filter((s) => AMBIENT.includes(s.path));
const egressObserved = ambient.length > 0;

if (egressObserved) {
  check("every ambient request carried `Origin: null` — opaque origin, read from the wire not from location.origin",
    ambient.every((s) => s.origin === "null"),
    { ambient: ambient.map((s) => s.path), origins: ambient.map((s) => s.origin), locationOriginInFrame: result?.result?.origin });
  check("AMBIENT XHR REACHES the endpoint (egress is not confined to the bridge)", arrived("/probe-plain"), seen.map((s) => s.path));
  ...
  // The policy is doing its job (or the probes did not run). Say which world this is,
  // and require the apparatus to be demonstrably alive before calling it a pass.
  const bridgedRefused = /refused|error/i.test(String(cases.bridged));
  const apparatusAlive = result?.ok === true && bridgedRefused === false ? false : (result?.ok === true);
...
// jfpx finding 1: /sandbox/i also matches CHROME'S OWN opaque-origin refusal
// ("The document is sandboxed…"), so disabling the localStorage shim left this
// green. The assertion now requires OUR teaching sentence, which Chrome never
// emits — the case says "our guard spoke", not "something threw".
check("storage teaching guards stay ACTIVE inside an imported module (all six throw OUR teaching error, not Chrome's native refusal)",
  guardKeys.every((k) => /^threw:/.test(String(G[k] ?? "")) && /is unavailable inside the script sandbox/.test(String(G[k] ?? ""))),
...
// ── the seam assertions (jfpx) ─────────────────────────────────────────────
const seams = await runSeamCases();
await Deno.writeTextFile(`${OUT}/seams.json`, JSON.stringify(seams, null, 1));
check("URL validation: the private/loopback predicate separates private from public",
  seams?.hosts?.loopback === true && seams?.hosts?.metadata === true && seams?.hosts?.ipv6Loopback === true && seams?.hosts?.public === false,
  seams?.hosts);
check("URL validation is what refuses a metadata URL — its OWN words, not the gate's",
  seams?.privateUrl?.ok === false && /private or loopback address/.test(String(seams?.privateUrl?.error)),
  seams?.privateUrl);
check("…and a public host on the approved list is ALLOWED, so the check is not a blanket refusal",
  seams?.publicAllowed?.ok === true, seams?.publicAllowed);
check("…and the same public host WITHOUT an approved run is refused by the GATE, whose words differ",
  seams?.gateRefusal?.ok === false && /no approved script run is active/.test(String(seams?.gateRefusal?.error)),
  seams?.gateRefusal);
check("LIB digest verifier (the second of three): a wrong digest refuses BEFORE minting",
  seams?.libWrongDigest?.resolved !== true && /digest/i.test(String(seams?.libWrongDigest?.threw ?? seams?.libWrongDigest?.value)) && seams?.libWrongDigest?.mints === 0,
  seams?.libWrongDigest);
