...
const HUB_PROBE = `(() => {
  ...
  return {
    ...
    panelHeadPaddingLeft: cs(document.querySelector('#jobs-section .panel-head'))?.paddingLeft,
    panelBodyPaddingLeft: cs(board)?.paddingLeft,
...
// The LEFT inset specifically — the one this check has always compared, and the
// one its variable names claim. (It used to be read as `padding.split(" ")[1]`,
// which is the shorthand's SECOND value, i.e. the RIGHT inset: equal to the left
// for the current 2-value `padding: 12px 14px`, and a different number for any
// box whose horizontal padding differs side to side.)
const insetPx = (v: unknown): number | null => {
  if (typeof v !== "string" || v.trim() === "") return null;
  const px = parseFloat(v);
  return Number.isFinite(px) ? px : null;
};
// PRESENCE IS THE PRECONDITION OF THE COMPARISON, not a detail of it.
// `parseFloat(String(padding || "0")… ?? "0")` turned ABSENCE into a value on
// both sides, so with neither `#jobs-board-host` nor `#jobs-section .panel-head`
// on the page this check compared 0 to 0, held, and reported safety
// (chrome-agent-platform-cgei: measured by the reviewer against a copy with both
// elements renamed away, and reproduced here — the pre-fix driver reports
// 38 passed / 8 failed on that copy and this check is NOT among the failures,
// while its rect-based sibling `padAlign` is). An unreadable inset is null, so
// the comparison goes RED instead.
const bodyLeft = insetPx(pad1440.panelBodyPaddingLeft);
const headLeft = insetPx(pad1440.panelHeadPaddingLeft);
check("hub: the Jobs board's content is inset like its own panel header",
  bodyLeft !== null && headLeft !== null && Math.abs(bodyLeft - headLeft) <= 1,
  ...
  return {
    ...
    buttons: btns.map((b) => {
      ...
      return {
        ...
        // Whether the label is THERE at all. labelClipped is false when the
        // element is missing, which is indistinguishable from "present and not
        // clipped" — so the 400px direction of the collapsed-rule check below
        // was satisfied by a chip with no label on it
        // (chrome-agent-platform-qdjn). Presence is measured, and required.
        hasLabel: !!label,
...
panelResults["260"].buttons = []; // M3 MUTATION: empty probe at 260px only
...
for (const w of PANEL_WIDTHS) {
  ...
  // The stress checks are about the LABEL, so a chip whose label is missing
  // satisfies "does not wrap" and "does not exceed" by having nothing to
  // measure (chrome-agent-platform-qdjn). Required here too.
  check(`panel ${w}px (long name): the label stays on one line OR is hidden`,
    b.hasLabel && (b.labelClipped || !b.wrapped), b);
  ...
  check(`panel ${w}px (long name): the chip does not exceed the container width`,
    b.hasLabel && (b.labelClipped || b.w <= r.containerWidth + 1),
    { w: b.w, containerWidth: r.containerWidth, hasLabel: b.hasLabel });
}

if (panelHasButtons) {
  for (const w of PANEL_WIDTHS) {
    const r = panelResults[String(w)];
    // NON-VACUITY, RE-TAKEN PER WIDTH (chrome-agent-platform-qdjn).
    // `panelHasButtons` above is measured at 300px ONLY, and every `.every()`
    // below is true on an empty list — so a probe that returned no buttons at
    // 260 or 400 made three checks at that width pass with nothing on the page.
    // (Measured on the pre-fix driver: with `buttons: []` at 260px, all three
    // 260px checks PASSED.) A guard inherited from another width is not a guard
    // for this one; this one is taken where it is used.
    const rendered = !r.error && Array.isArray(r.buttons) && r.buttons.length > 0 &&
      r.buttons.every((b: any) => b.w > 0 && b.h > 0);
    check(`panel ${w}px: the harness buttons rendered at this width (non-vacuity)`, rendered,
      { error: r.error ?? null, containerVisible: r.containerVisible, containerWidth: r.containerWidth,
        buttons: (r.buttons ?? []).map((b: any) => ({ t: b.text, w: b.w, h: b.h })) });
    check(`panel ${w}px: no harness button's text overflows its own box`,
      rendered && r.buttons.every((b: any) => b.textOverflowsBy <= 1), { rendered, buttons: r.buttons });
    check(`panel ${w}px: no harness button wraps its label to more than one line`,
      rendered && r.buttons.every((b: any) => b.hasLabel && (b.labelClipped || !b.wrapped)),
      { rendered, buttons: r.buttons.map((b: any) => ({ t: b.text, h: b.h, singleLine: b.singleLine, wrapped: b.wrapped, hasLabel: b.hasLabel })) });
    ...
    check(`panel ${w}px: the collapsed rule ${expectClipped ? "hides the label" : "leaves the label visible"} (the container query ${expectClipped ? "fires" : "does not fire"})`,
      rendered && r.buttons.every((b: any) => b.hasLabel && b.labelClipped === expectClipped),
      { rendered, expectClipped, containerWidth: r.containerWidth,
        buttons: r.buttons.map((b: any) => ({ t: b.text, labelClipped: b.labelClipped, hasLabel: b.hasLabel, w: b.w, h: b.h })) });
    check(`panel ${w}px: every harness button stays inside its container`,
      rendered && r.buttons.every((b: any) => !b.overflowsContainer), { rendered, containerWidth: r.containerWidth, buttons: r.buttons });
  }
  // At the collapsed width the mark must carry the identity — and the width the
  // mark is judged AT needs its own non-vacuity guard, for the same reason.
  const collapsed = panelResults["260"];
  const collapsedRendered = !collapsed.error && Array.isArray(collapsed.buttons) && collapsed.buttons.length > 0;
  check("panel 260px: every harness button carries a mark",
    collapsedRendered && collapsed.buttons.every((b: any) => b.hasMark && b.markW >= 12),
    { collapsedRendered, buttons: (collapsed.buttons ?? []).map((b: any) => ({ t: b.text, hasMark: b.hasMark, markW: b.markW, tag: b.markTag, ns: b.markNS })) });
  check("panel 260px: the accessible name still states the full harness name",
    collapsedRendered && collapsed.buttons.every((b: any) => (b.ariaLabel || "").length > 4),
    { collapsedRendered, buttons: (collapsed.buttons ?? []).map((b: any) => b.ariaLabel) });
}
