// zz-hg03-store-restart-probe.ts — MY verification extension of the landed
// harness (ds-flash-2, 2026-09-18). It is the landed instrument PLUS one phase:
// after the page-reload check, stop the extension's service worker and reopen
// the task, to ask whether the transcript is re-read from the STORE rather than
// served from a warm worker. The landed file is untouched; this copy exists only
// in my verification worktree and its extra checks are labelled.
//
// (copy of) cap-evidence/acp-journal-acceptance.ts — REAL-BROWSER acceptance for
...
try {
  ...
  // ── MY PHASE: does the transcript survive a SERVICE WORKER RESTART? ─────
  // The landed check reloads the PAGE, which a warm worker can serve from memory.
  // This stops the worker, waits, then reopens the same task.
  try {
    // the landed harness drives raw CDP, so find the worker target by URL
    const swTarget = async () => {
      const t = await send("Target.getTargets");
      return (t?.targetInfos ?? []).find((x: any) => /service-worker\.js/.test(String(x.url ?? "")) && x.type === "service_worker") ?? null;
    };
    const beforeStop = await swTarget();
    // The ServiceWorker domain must be ENABLED before its commands exist.
    let stoppedBy = "none";
    try {
      await send("ServiceWorker.enable");
      await send("ServiceWorker.stopAllWorkers");
      stoppedBy = "cdp:ServiceWorker.stopAllWorkers";
    } catch (e) {
      stoppedBy = `cdp failed (${String(e).slice(0, 60)}) — falling back to idle suspension`;
      // Fallback: an extension worker suspends after ~30s idle; closing every
      // extension page and waiting forces that, and the reopen then starts a
      // FRESH worker that must re-read the store from disk.
      await send("Target.closeTarget", { targetId: ntp }).catch(() => {});
      await sleep(40_000);
    }
    await sleep(3000);
    const afterStop = await swTarget();
    console.log(`  [my phase] worker stop mechanism: ${stoppedBy}`);
    // NOT an assertion about the product: this attempt cannot force a stop here
    // (the ServiceWorker CDP domain is unavailable on this endpoint and the open
    // bridge WebSocket keeps the worker warm), so it is recorded as an
    // observation. PHASE 2 below is the conclusive one.
    console.log(`  [my phase] attempt 1 (warm): worker before=${beforeStop?.targetId ?? null} after=${afterStop?.targetId ?? null} — ${(!afterStop || afterStop.targetId !== beforeStop.targetId) ? "restarted" : "STILL THE SAME WORKER, so the reload below is served by a warm worker"}`);
    let page = ntp;
    try { await send("Page.reload", {}, ntp); } catch {
      const t = await send("Target.createTarget", { url: "chrome-extension://" + extId + "/ntp/ntp.html" });
      page = t.targetId;
      await send("Target.attachToTarget", { targetId: page, flatten: true }).catch(() => {});
    }
    const ntp2 = page;
    await sleep(3500);
    const afterRestart = await evl(ntp2, `(() => { const nav = document.getElementById('thread-sidebar');
      return nav ? [...nav.querySelectorAll('*')].map(n => (n.textContent||'').trim()).filter(Boolean).slice(0, 12) : null; })()`);
    await shot(ntp2, "04-after-sw-restart");
    check("MY PHASE: after the worker restart the task is STILL listed (read from the store, not memory)",
      Array.isArray(afterRestart) && afterRestart.some((x) => x.includes("ACP journal probe")), afterRestart);
    if (task?.id) {
      const opened2 = await clickExpr(ntp2, `(() => { const nav = document.getElementById('thread-sidebar');
        if (!nav) return null;
        return [...nav.querySelectorAll('*')].find(n => n.children.length === 0 && (n.textContent||'').includes('ACP journal probe')) ?? null; })()`);
      await sleep(3000);
      const reopened2 = await evl(ntp2, `(() => { const c = document.getElementById('thread-conversation');
        return c ? [...c.querySelectorAll('message-bubble')].map(b => ({ role: b.getAttribute('role'), text: (b.getAttribute('content')||'').slice(0,200) })) : null; })()`);
      await shot(ntp2, "05-reopened-after-sw-restart");
      const text2 = (reopened2 ?? []).map((b: any) => b.text).join(" ");
      check("MY PHASE: the transcript survives the worker restart", /ACP journal probe/i.test(text2) && /fake reply/i.test(text2), { opened2, bubbles: (reopened2 ?? []).map((b: any) => b.role) });
    }

    // ── attempt 2: cut what keeps the worker WARM ──────────────────────────
    // An extension SW is kept alive by open ports and extension pages. The
    // bridge's WebSocket is one, so shut the bridge down, close every page,
    // wait past the 30s idle window, and see whether the worker target is gone.
    try {
      await bridge.shutdown().catch(() => {});
      await send("Target.closeTarget", { targetId: ntp2 }).catch(() => {});
      await sleep(45_000);
      const afterIdle = await swTarget();
      const restarted = !afterIdle || afterIdle.targetId !== beforeStop.targetId;
      check("MY PHASE 2: with no open port and no page, the worker really goes away (or say so)",
        restarted, { workerAfterIdle: afterIdle?.targetId ?? null, before: beforeStop?.targetId ?? null });
      // reopen from cold and ask the store again
      const t2 = await send("Target.createTarget", { url: "chrome-extension://" + extId + "/ntp/ntp.html" });
      // raw CDP: a target id is not a session id — attach to get one
      const a2 = await send("Target.attachToTarget", { targetId: t2.targetId, flatten: true });
      const ntp3 = a2.sessionId;
      await send("Runtime.enable", {}, ntp3).catch(() => {});
      await sleep(4000);
      const afterColdStart = await evl(ntp3, `(() => { const nav = document.getElementById('thread-sidebar');
        return nav ? [...nav.querySelectorAll('*')].map(n => (n.textContent||'').trim()).filter(Boolean).slice(0, 12) : null; })()`);
      await shot(ntp3, "06-after-cold-start");
      check("MY PHASE 2: after a COLD start the task is still listed (this is the store, not memory)",
        Array.isArray(afterColdStart) && afterColdStart.some((x: string) => x.includes("ACP journal probe")), afterColdStart);
      if (task?.id) {
        await clickExpr(ntp3, `(() => { const nav = document.getElementById('thread-sidebar');
          if (!nav) return null;
          return [...nav.querySelectorAll('*')].find(n => n.children.length === 0 && (n.textContent||'').includes('ACP journal probe')) ?? null; })()`);
        await sleep(3500);
        const cold = await evl(ntp3, `(() => { const c = document.getElementById('thread-conversation');
          return c ? [...c.querySelectorAll('message-bubble')].map(b => ({ role: b.getAttribute('role'), text: (b.getAttribute('content')||'').slice(0,200) })) : null; })()`);
        await shot(ntp3, "07-transcript-after-cold-start");
        const text3 = (cold ?? []).map((b: any) => b.text).join(" ");
        check("MY PHASE 2: the transcript survives the COLD start", /ACP journal probe/i.test(text3) && /fake reply/i.test(text3), { bubbles: (cold ?? []).map((b: any) => b.role) });
      }
    } catch (err) {
      check("MY PHASE 2: the cold-start phase ran", false, String(err).slice(0, 200));
    }
  } catch (err) {
    check("MY PHASE: the worker-restart phase ran", false, String(err));
  }
