...
Deno.test("bgagent delete: NTP row uses open-delete with the confirm + deterministic recipe:<id> cancel", async () => {
  ...
  // the row's delete flow: confirm → task.cancel with the recipe: name
  assertMatch(
    src,
    /addEventListener\("delete"[\s\S]{0,900}?task\.cancel", \{ name: `recipe:\$\{a\.id\}` \}/,
    "row delete must confirm then cancel the deterministic recipe:<id> task",
  );
  // the NTP delete-agent button (thread-view header) has the SAME fix
  assertMatch(
    src,
    /else if \(kind === "background"\) \{[\s\S]{0,500}?task\.cancel", \{ name: `recipe:\$\{id\}` \}/,
    "the header delete path cancels recipe:<id>, never the raw id",
  );
  // no stale raw-id cancel remains for background agents
