...
} from "../extension/lib/agent-tools-filter.js";
...
Deno.test("bundled wasm filtering: respects agent tools allowlist", () => {
  ...
  // 1. Unfiltered (null): every shipped bundled package is admitted. The count
  // is DERIVED from the inventory — a literal here went stale the moment the
  // catalogue grew (28 at authoring; the bead's whole lesson is that a stale
  // literal is a silent lie), so assert the property, not the number.
  ...
  assertEquals(allRecords.length, BUNDLED_TOOL_PACKAGE_ROWS.length, "unfiltered produces every shipped bundled tool row");

  // 2. Filtered allowlist by toolId — through the PRODUCTION filter (the same
  // function the service worker's run builder uses), not a local copy.
  const toolAllowList = ["grep", "diff"];
  const filteredRecords1 = executableBundledToolRecords(
    filterBundledWasmRowsForAgent({ bundledWasm: toolAllowList }, BUNDLED_TOOL_PACKAGE_ROWS),
    {
      scope,
      sourceGeneration: `bundled-inventory:${BUNDLED_INVENTORY.release}`,
      closureGeneration: "task-execution-core",
  ...
  const packageAllowList = ["cap.bundled.csvtool", "cap.bundled.awk.filter.bounded"];
  const filteredRecords2 = executableBundledToolRecords(
    filterBundledWasmRowsForAgent({ bundledWasm: packageAllowList }, BUNDLED_TOOL_PACKAGE_ROWS), {
    scope,
    sourceGeneration: `bundled-inventory:${BUNDLED_INVENTORY.release}`,
    closureGeneration: "task-execution-core",
  });
  ...
  // 4. Empty allowlist: zero tools (through the production filter).
  const emptyRecords = executableBundledToolRecords(
    filterBundledWasmRowsForAgent({ bundledWasm: [] }, BUNDLED_TOOL_PACKAGE_ROWS),
    {
      scope,
      sourceGeneration: `bundled-inventory:${BUNDLED_INVENTORY.release}`,
      closureGeneration: "task-execution-core",
...
Deno.test("WebMCP origin allowlist filtering: the production filter enforces allowed origins", () => {
  // Drives filterWebmcpOriginsForAgent — the exact function the service worker
  // uses to decide which site origins an agent's run may build workers for.
  // (The preserved test re-implemented the filter inline; deleting the
  // production filter would have left it green.)
  const allOrigins = ["https://github.com", "https://linear.app", "https://notion.so"];

  // 1. Unrestricted agent (tools == null)
  assertEquals(
    filterWebmcpOriginsForAgent(null, allOrigins),
    allOrigins,
    "unrestricted agent sees all enrolled origins",
  );

  // 2. Restricted agent (webmcpOrigins = ["https://github.com"])
  assertEquals(
    filterWebmcpOriginsForAgent({ webmcpOrigins: ["https://github.com"] }, allOrigins),
    ["https://github.com"],
    "restricted agent sees only allowed origins",
  );
  // A non-allowlisted origin is EXCLUDED from worker enrollment — and the
  // service worker's delegateTask guard (the second gate, service-worker.js
  // `delegateGuard`) denies it at dispatch time through the SAME predicate.
  const githubOnly = filterWebmcpOriginsForAgent({ webmcpOrigins: ["https://github.com"] }, allOrigins);
  assert(!githubOnly.includes("https://linear.app"), "non-allowlisted origin is unreachable");
  const allowlist = webmcpAllowlistForAgent({ webmcpOrigins: ["https://github.com"] });
  assert(isWebmcpOriginAllowed(allowlist, "https://github.com"), "the guard admits the allow-listed origin");
  assert(!isWebmcpOriginAllowed(allowlist, "https://linear.app"), "the guard denies a non-allowlisted origin");
  assert(isWebmcpOriginAllowed(webmcpAllowlistForAgent(null), "https://linear.app"), "no allow-list means unrestricted");

  // 3. Empty allowlist (webmcpOrigins = []): a closed list with no members.
  const originsEmpty = filterWebmcpOriginsForAgent({ webmcpOrigins: [] }, allOrigins);
