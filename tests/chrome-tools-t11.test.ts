...
Deno.test("T11 management permission: reads fail closed without the management permission", async () => {
  ...
  assertEquals(noPerm.error, "management permission not granted — allow it in the approval card here, or in Settings → Permissions");
  ...
  assertEquals(getNoPerm.error, "management permission not granted — allow it in the approval card here, or in Settings → Permissions");
  const warnNoPerm = await t.get_extension_permission_warnings.execute({ id: "other-extension-id" });
  assertEquals(warnNoPerm.error, "management permission not granted — allow it in the approval card here, or in Settings → Permissions");
});
