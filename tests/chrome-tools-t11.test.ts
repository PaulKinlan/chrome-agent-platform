...
Deno.test("T11 GLOBAL-grant-only: sidePanel + action mutations refuse an origin grant, accept the GLOBAL grant", async () => {
  reset();
  grantedPermissions.add("sidePanel");
  const t = tools();
  // Origin grant is never enough for these browser-wide own-surface mutations.
  await setOriginBrowserControlGrant(["https://example.com"]);
  const panelOrigin = await t.set_panel_behavior.execute({ openPanelOnActionClick: true });
  assertEquals(panelOrigin.error, "browser control not granted for this browser-wide change — ask the user to approve it in Settings");
  const actionOrigin = await t.disable_action.execute({});
  assertEquals(actionOrigin.error, "browser control not granted for this browser-wide change — ask the user to approve it in Settings");
  assertEquals(chromeCalls.filter((c) => c[0] === "sidePanel.setPanelBehavior").length, 0);
  assertEquals(chromeCalls.filter((c) => c[0] === "action.disable").length, 0);
  // The GLOBAL grant authorizes them.
  await setGlobalBrowserControlGrant();
  const panelOk = await t.set_panel_behavior.execute({ openPanelOnActionClick: true });
  assertEquals(panelOk.ok, true);
  assertEquals(panelBehavior.openPanelOnActionClick, true);
  const disableOk = await t.disable_action.execute({});
  assertEquals(disableOk.ok, true);
  assertEquals(actionState.enabled, false);
  const enableOk = await t.enable_action.execute({ tabId: 7 });
  assertEquals(enableOk.ok, true);
  assertEquals(actionState.enabled, true);
  await revokeBrowserControlGrant();
});

Deno.test("T11 sidePanel permission: set_side_panel_options + set_panel_behavior fail closed without sidePanel", async () => {
  reset();
  await setGlobalBrowserControlGrant();
  const t = tools();
  const noPerm = await t.set_side_panel_options.execute({ path: "panel.html" });
  assertEquals(noPerm.error, "sidePanel permission not granted — enable the Side panel in Settings");
  assertEquals(chromeCalls.filter((c) => c[0] === "sidePanel.setOptions").length, 0);
  const behaviorNoPerm = await t.set_panel_behavior.execute({ openPanelOnActionClick: true });
  assertEquals(behaviorNoPerm.error, "sidePanel permission not granted — enable the Side panel in Settings");
  const getNoPerm = await t.get_side_panel_options.execute({});
  assertEquals(getNoPerm.error, "sidePanel permission not granted — enable it in Settings (Side panel)");
  await revokeBrowserControlGrant();
});

Deno.test("T11 side-panel path confinement: traversal / absolute / scheme / %-encoded paths are refused (fail closed)", async () => {
  reset();
  grantedPermissions.add("sidePanel");
  await setGlobalBrowserControlGrant();
  const t = tools();
  const hostile = [
    "../evil.html",            // traversal
    "..\\evil.html",           // backslash traversal
    "/abs/panel.html",         // absolute path
    "http://evil.example/p",   // URL scheme
    "https://evil.example/p",  // URL scheme
    "chrome://settings",       // chrome scheme
    "data:text/html,x",        // data scheme
    "%2e%2e/evil.html",        // %-encoded traversal
    "panel%2ehtml",            // any %-encoding refused
    "a//../b",                 // embedded traversal
    "sidepanel/../secret.html",// mid-path traversal
  ];
  for (const path of hostile) {
    const res = await t.set_side_panel_options.execute({ path });
    assert(res.error, `hostile path refused: ${path}`);
    assertEquals(res.error, "side panel path must be an extension-relative bundled page (no absolute, scheme, traversal, or %-encoded paths)", `path: ${path}`);
  }
  assertEquals(sidePanelSetCalls.length, 0, "no sidePanel.setOptions for a hostile path");
  // A clean extension-relative bundled page is accepted.
  const ok = await t.set_side_panel_options.execute({ path: "panel/page.html", enabled: true });
  assertEquals(ok.ok, true);
  assertEquals(ok.path, "panel/page.html");
  assertEquals(panelOptions.path, "panel/page.html");
  await revokeBrowserControlGrant();
});

Deno.test("T11 set_side_panel_options: schema requires path and/or enabled; get_side_panel_options reads", async () => {
  reset();
  grantedPermissions.add("sidePanel");
  const t = tools();
  // Neither path nor enabled → refused by the schema refine.
  assertEquals(t.set_side_panel_options.inputSchema.safeParse({}).success, false);
  assertEquals(t.set_side_panel_options.inputSchema.safeParse({ path: "panel.html" }).success, true);
  assertEquals(t.set_side_panel_options.inputSchema.safeParse({ enabled: false }).success, true);
  // get_side_panel_options reads without a grant (read-only).
  const opts = await t.get_side_panel_options.execute({});
  assertEquals(opts.path, "sidepanel/sidepanel.html");
