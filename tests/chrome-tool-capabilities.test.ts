...
Deno.test("chrome capability table is exact and complete for 60 browser + 29 management tools", () => {
  ...
  assertEquals(CHROME_TOOL_CAPABILITY_TABLE.length, 89);
  assertEquals(CHROME_TOOL_CAPABILITY_TABLE.filter((row) => row.sourceKind === "chrome-api").length, 60);
...
Deno.test("catalog descriptors consume exact canonical capabilities and capability digests", () => {
  ...
  assertEquals(catalog.descriptors.length, 89);
...
Deno.test("unbound lazy browser/management records preserve source closure and validator custody without invocation", async () => {
  ...
  assertEquals(browserRecords.length, 60);
...
Deno.test("shadow capture discloses bounded selected capability summaries and only a non-selected count", async () => {
  ...
  assertEquals(capture.nonSelectedCount, 89 - capture.selectedCount);
...
Deno.test("unsafe-for-cutover list remains policy metadata and does not filter the 89-record catalog", () => {
  ...
  assertEquals(CHROME_TOOL_CAPABILITY_TABLE.length, 89);
  assertEquals(new Set(CHROME_TOOL_CAPABILITY_TABLE.map((row) => row.toolName)).size, 89);
});
