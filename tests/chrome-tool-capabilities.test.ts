...
Deno.test("chrome capability table is exact and complete for 130 browser + 29 management tools", () => {
  ...
  assertEquals(CHROME_TOOL_CAPABILITY_TABLE.filter((row) => row.sourceKind === "chrome-api").length, 130);
...
Deno.test("unbound lazy browser/management records preserve source closure and validator custody without invocation", async () => {
  ...
  assertEquals(browserRecords.length, 130);
