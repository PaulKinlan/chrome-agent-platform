...
// ──────────────────────────────────────────────────────────────────────────
Deno.test("shadow summary: the full BUNDLED row set projects into the catalog (37 rows in toolsBySource[bundled-package])", async () => {
  const { BUNDLED_TOOL_PACKAGE_ROWS } = await import("../extension/lib/bundled-tool-packages.data.js");
  ...
  const summary = await controller.inspect({ action: "summary" });
  const bundled = summary.toolsBySource["bundled-package"] ?? [];
  assertEquals(bundled.length, 37, "all 37 bundled rows are listed");
  assertEquals(summary.bySource["bundled-package"], 37, "the bySource count matches");
