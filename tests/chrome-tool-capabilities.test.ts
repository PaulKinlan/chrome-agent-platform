...
Deno.test("chrome capability table is exact and complete for 43 browser + 29 management tools", () => {
  ...
  assertEquals(CHROME_TOOL_CAPABILITY_TABLE.length, 72);
  assertEquals(CHROME_TOOL_CAPABILITY_TABLE.filter((row) => row.sourceKind === "chrome-api").length, 43);
...
Deno.test("catalog descriptors consume exact canonical capabilities and capability digests", () => {
  ...
  assertEquals(catalog.descriptors.length, 72);
...
Deno.test("unbound lazy browser/management records preserve source closure and validator custody without invocation", async () => {
  ...
  assertEquals(browserRecords.length, 43);
...
Deno.test("shadow capture discloses bounded selected capability summaries and only a non-selected count", async () => {
  ...
  assertEquals(capture.nonSelectedCount, 72 - capture.selectedCount);
...
Deno.test("unsafe-for-cutover list remains policy metadata and does not filter the 72-record catalog", () => {
  ...
  assertEquals(CHROME_TOOL_CAPABILITY_TABLE.length, 72);
  assertEquals(new Set(CHROME_TOOL_CAPABILITY_TABLE.map((row) => row.toolName)).size, 72);
});
