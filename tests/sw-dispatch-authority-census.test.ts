...
//   3. The census classification is complete, disjoint, and covers 100% of registered routes (260 total).
...
Deno.test("census: all registered routes in handlers are derived via AST and total 260", () => {
  const registered = extractAllRegisteredRoutes();
  assertEquals(registered.size, 260, `registered routes population must equal 260 (got ${registered.size})`);
});
