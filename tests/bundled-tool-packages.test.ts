...
Deno.test("manifests: all 36 shipped manifests validate against the real authority (canonical bytes)", async () => {
  const probe = new WasmPackageAuthority();
  assertEquals(BUNDLED_INVENTORY.manifests.length, 36);
...
Deno.test("admission: all 36 packages admit through the real authority over the real bytes; re-admission dedupes", async () => {
  ...
  assertEquals(first.results.length, 36);
...
Deno.test("posture: descriptors admit exactly the 36-tool Settings allowlist", () => {
  assertEquals(BUNDLED_TOOL_PACKAGES.length, 36);
  assertEquals(new Set(BUNDLED_TOOL_PACKAGES.map((r) => r.packageId)).size, 36);
  assertEquals(new Set(BUNDLED_TOOL_PACKAGES.map((r) => r.toolId)).size, 36);
...
Deno.test("store map: exact archivePath→executable mapping for ALL 36 shipped CAS binaries", async () => {
  const map = await buildBundledWasmManifestMap(repoRoot);
  assertEquals(map.size, 36);
