...
Deno.test("manifests: all 33 shipped manifests validate against the real authority (canonical bytes)", async () => {
...
Deno.test("admission: all 33 packages admit through the real authority over the real bytes; re-admission dedupes", async () => {
...
Deno.test("posture: descriptors admit exactly the 33-tool Settings allowlist", () => {
  ...
  assertEquals(JSON.stringify(previewRows.map((r) => r.toolId).sort()), JSON.stringify(
    ["awk", "awk_filter_bounded", "base64", "compressops", "csvtool", "cut", "date_formatter_bounded", "diff", "du", "grep", "gzip", "head", "imageops", "jq", "markdown", "md5sum", "patch", "sed", "sha256sum", "sha512sum", "sort", "sqlite3_query_bounded", "stat", "tail", "toml2json", "touch", "tr", "tree", "truncate", "uniq", "uuid", "wc", "xxd"],
  ), "exactly the 33-tool allowlist");
...
Deno.test("store map: exact archivePath→executable mapping for ALL 33 shipped CAS binaries", async () => {
