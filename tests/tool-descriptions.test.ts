...
Deno.test("tool naming & descriptions: all 36 tools have Unix-name displayName and lead with '<toolname> - '", () => {
  assertEquals(BUNDLED_TOOL_PACKAGE_ROWS.length, 36, "exact 36 bundled tool rows");
  assertEquals(Object.keys(AGENT_DESCRIPTIONS).length, 36, "exact 36 agent descriptions");
...
Deno.test("manifest descriptions: all 36 manifests contain matching agent-useful descriptions", async () => {
  for (const row of BUNDLED_TOOL_PACKAGE_ROWS) {
