...
Deno.test("tool naming & descriptions: all 37 tools have Unix-name displayName and lead with '<toolname> - '", () => {
  assertEquals(BUNDLED_TOOL_PACKAGE_ROWS.length, 37, "exact 37 bundled tool rows");
  assertEquals(Object.keys(AGENT_DESCRIPTIONS).length, 37, "exact 37 agent descriptions");
...
Deno.test("manifest descriptions: all 37 manifests contain matching agent-useful descriptions", async () => {
  for (const row of BUNDLED_TOOL_PACKAGE_ROWS) {
