...
Deno.test("tool naming & descriptions: all 39 tools have Unix-name displayName and lead with '<toolname> - '", () => {
  assertEquals(
    BUNDLED_TOOL_PACKAGE_ROWS.length,
    39,
    "exact 39 bundled tool rows",
  );
  assertEquals(
    Object.keys(AGENT_DESCRIPTIONS).length,
    39,
    "exact 39 agent descriptions",
  );
...
Deno.test("manifest descriptions: all 39 manifests contain matching agent-useful descriptions", async () => {
  for (const row of BUNDLED_TOOL_PACKAGE_ROWS) {
...
Deno.test("tool naming & descriptions: importing the bundled-tool generator writes nothing (chrome-agent-platform-i1i9)", async () => {
  ...
  assertEquals(
    ...
    "the pure surface is the one data export, and all 39 descriptions survive",
  );
  assertEquals(
    await fingerprint(),
    before,
