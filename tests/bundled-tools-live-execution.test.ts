...
Deno.test("ten9 census: 36 admitted bundled tools; the formerly preview-only 24 are named and none is gated", () => {
  assertEquals(PREVIEW_TOOL_IDS.length, 36, "exactly 36 bundled tools admitted in catalog");
  ...
  assertEquals(formerlyPreviewOnly.length, 26, "the 24 un-gated tools plus oxipng (m3vb) and jxl (agpu) are present");
...
Deno.test("ten9 GUARD: live dispatch of EVERY admitted bundled tool reaches the executor — preview_only_tool must never return", async () => {
  ...
  assertEquals(records.length, 37, "every admitted tool exposes an executable record (36 wasi + 1 call-export)");
