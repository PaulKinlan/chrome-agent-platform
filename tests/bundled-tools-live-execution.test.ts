...
Deno.test("ten9 census: 36 admitted bundled tools; the formerly preview-only 24 are named and none is gated", () => {
  assertEquals(PREVIEW_TOOL_IDS.length, 37, "exactly 37 bundled tools admitted in catalog");
  ...
  assertEquals(formerlyPreviewOnly.length, 27, "the 24 un-gated tools plus oxipng (m3vb), jxl (agpu) and avif (ou4x), all admitted straight into the job lane, are present");
