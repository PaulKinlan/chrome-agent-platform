...
Deno.test("p15i reconciliation: a killed run's exact fixture residue self-heals, real drift is never masked", () => {
  const headBytes = headProvenanceBytes();
  ...
  try {
    ...
    assertEquals(verify().code, 0, "verify passes again after reconciliation");
    ...
    assertNotEquals(verify().code, 0, "real drift still fails closed after reconciliation");
  } finally {
    Deno.writeFileSync(DRIFT_TARGET, original);
    try { Deno.removeSync(stray); } catch { /* absent */ }
  }
  assertEquals(verify().code, 0, "clean state restored");
});
