...
Deno.test("list_tools: enumerates all categories including the 39 admitted bundled Wasm tools", async () => {
  ...
  assertEquals(result.counts.bundledWasm, 39, "must report exactly 39 bundled Wasm tools");
  assertEquals(result.tools["bundled-wasm"].length, 39, "must list all 39 bundled Wasm tools");
...
Deno.test("list_tools: category filter returns only the requested category", async () => {
  ...
  assertEquals(filtered.tools["bundled-wasm"].length, 39);
...
Deno.test("list_tools: returns every complete descriptor without the retired 32 KiB truncation cap", async () => {
  ...
  assertEquals(result.tools["bundled-wasm"].length, 39);
...
Deno.test("master-skill: operating manual truthfully includes list_tools and the 39 bundled Wasm tools", () => {
  ...
  assert(MASTER_SKILL.includes("39 on-device bundled Wasm tools"), "MASTER_SKILL must mention 39 bundled Wasm tools");
