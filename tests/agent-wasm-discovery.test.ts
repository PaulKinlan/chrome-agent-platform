...
Deno.test("list_tools: enumerates all categories including the 33 admitted bundled Wasm tools", async () => {
  ...
  assertEquals(result.counts.bundledWasm, 33, "must report exactly 33 bundled Wasm tools");
  assertEquals(result.tools["bundled-wasm"].length, 33, "must list all 33 bundled Wasm tools");
...
Deno.test("master-skill: operating manual truthfully includes list_tools and the 33 bundled Wasm tools", () => {
  ...
  assert(MASTER_SKILL.includes("33 on-device bundled Wasm tools"), "MASTER_SKILL must mention 33 bundled Wasm tools");
