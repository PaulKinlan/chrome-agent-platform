...
Deno.test("preview: an UNKNOWN toolId fails closed (the static allowlist is exact)", () => {
  ...
  assertEquals(JSON.stringify(PREVIEW_TOOL_IDS), JSON.stringify(
    ["awk", "awk_filter_bounded", "base64", "csvtool", "cut", "date_formatter_bounded", "diff", "du", "grep", "gzip", "hasher", "head", "imageops", "jq", "markdown", "md5sum", "patch", "sed", "sha256sum", "sha512sum", "sort", "sqlite3_query_bounded", "stat", "tail", "toml2json", "touch", "tr", "tree", "truncate", "uniq", "uuid", "wc", "xxd"],
  ));
  for (const spec of Object.values(PREVIEW_SPECS)) {
