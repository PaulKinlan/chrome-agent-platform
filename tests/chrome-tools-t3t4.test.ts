...
// ──────────────────────────────────────────────────────────────────────────
Deno.test("T3/T4: browserToolset has exactly 126 tools matching BROWSER_TOOL_NAMES (T1/T2 + T8 + T13 + T5/T6 + T7 + T11 + T9 + T10 + T12 + 14)", () => {
  reset();
  const browser = tools();
  ...
  assertEquals(CHROME_TOOL_CAPABILITY_BOUNDS.totalTools, 164);
