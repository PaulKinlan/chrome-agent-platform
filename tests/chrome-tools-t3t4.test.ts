...
// ──────────────────────────────────────────────────────────────────────────
Deno.test("T3/T4: browserToolset has exactly 118 tools matching BROWSER_TOOL_NAMES (T1/T2 + T8 + T13 + T5/T6 + T7 + T11 + T9 + T10 + 14)", () => {
  reset();
  const browser = tools();
  assertEquals(Object.keys(browser), BROWSER_TOOL_NAMES);
  assertEquals(BROWSER_TOOL_NAMES.length, 118);
  assertEquals(CHROME_TOOL_CAPABILITY_BOUNDS.browserTools, 118);
  assertEquals(CHROME_TOOL_CAPABILITY_BOUNDS.totalTools, 147);
