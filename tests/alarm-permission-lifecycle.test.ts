...
Deno.test("alarm lifecycle: Settings owns request; worker owns activation", async () => {
  ...
  assertStringIncludes(optionsSource, 'type: "alarms.permission-granted"');
  assertStringIncludes(
    workerSource,
    'async "alarms.permission-granted"(_message, context)',
  );
  assertStringIncludes(workerSource, "requireSettingsSender(context)");
  assertStringIncludes(workerSource, 'if (id === "alarms")');
