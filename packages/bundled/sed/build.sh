...
# The resource dir must carry BOTH the wasm32 clang_rt builtins
# (lib/wasm32-unknown-wasip1/libclang_rt.builtins.a) and clang's builtin
# headers (include/) — the wasi-sdk-22 split layout supplies both.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")" && pwd)"
SYSROOT="${WASI_SYSROOT:?set WASI_SYSROOT to a wasi-sysroot-22.0 tree}"
RTDIR="${WASI_RT:?set WASI_RT to a resource dir with lib/wasm32-unknown-wasip1/libclang_rt.builtins.a + include/}"
export SOURCE_DATE_EPOCH=0 TZ=UTC LC_ALL=C
FLAGS="-O2 -DNDEBUG -g0 -ffile-prefix-map=$ROOT=/src -Wl,--strip-all -Wl,--max-memory=33554432"
