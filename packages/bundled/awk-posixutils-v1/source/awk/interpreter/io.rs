...
#[cfg(target_os = "wasi")]
impl WritePipes {
    pub fn write(&mut self, _command: AwkString, _contents: AwkString) -> Result<(), String> {
        Err("command pipes are unavailable in the WASI build".to_string())
    }
    pub fn flush_file(&mut self, _filename: &str) -> bool { false }
    pub fn flush_all(&mut self) -> bool { true }
    pub fn close_pipe(&mut self, _filename: &str) -> Option<i32> { None }
}
...
impl ReadPipes {
    pub fn read_next_record(
        &mut self,
        _command: AwkString,
        _separator: &RecordSeparator,
    ) -> Result<Option<String>, String> {
        Err("command pipes are unavailable in the WASI build".to_string())
    }
    pub fn close_pipe(&mut self, _command: &str) -> Option<i32> { None }
}
