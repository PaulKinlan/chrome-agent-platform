...
- **Durable run authority (`ac1c4fe`, release 0.2.110 candidate; pending integration)** — source-grounded execution ownership, OPFS key layout, outbox order, recovery states, exact native-quota compensation, and security boundaries are indexed in [docs/DURABLE-RUN-ARCHITECTURE.md](docs/DURABLE-RUN-ARCHITECTURE.md). Review iterations v1-v39 produced no final whole-product authority. Static checks do not prove the loaded UI; the documented seven-screenshot Chrome sequence has not been captured and is required before landing.
