...
The public lazy shadow is loaded-MV3 verified. Source-only OPFS and bundled-package authorities now own strict job records, exact inventory/CAS checks, bounded raw Wasm measurement and exact-generation package WAL records without an execution route or binary. The current source candidate adds retained code-diff artifacts: getter-safe Unicode path/change validation, producer+context+base/result identity, bounded digest-keyed CAS retention through the artifact WAL, and non-authoritative bounded views. Apply/reject/undo remain synchronous fail-closed stubs with no route or workspace authority. The loaded-MV3 runtime probe still blocks any executable consumer. After review: fresh-Worker execution host; provenance-clean bundled tools; a separately owner-approved diff mutation lifecycle; Chrome lazy tools; Tool Library; owner install only after distribution-policy resolution; spreadsheet tools; continuous abuse gates. Providers still receive the eager set and cannot call `search_tools` or `execute_tool`; provider cutover remains separate. The existing semantic-search task remains the sole semantic authority and is P0; embeddings/SQLite are not selected by the lexical slice.
