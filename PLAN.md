...
The next owner-decision-free slice is now in source review: a fixed two-tool lazy wire, selected-only Settings capture, and an injectable execution core that validates bounded arguments and repeatedly re-resolves live source/package/run/agent/origin/document authority around the existing dispatch closures. It is shadow-only: providers still receive the eager set and cannot call `search_tools` or `execute_tool`. Provider exposure waits for exact loaded-MV3 nondisclosure/uncallability evidence. After that gate: loaded-MV3 runtime probe; package authority and per-job OPFS workspaces; fresh-Worker execution host; provenance-clean bundled tools; diff artifacts; Chrome lazy tools; Tool Library; owner install only after distribution-policy resolution; spreadsheet tools; continuous abuse gates. The existing semantic-search task remains the sole semantic authority and is P0; embeddings/SQLite are not selected by the lexical slice.
