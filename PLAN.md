...
- [ ] **WebMCP discovery observability (Paul 2026-08-18) — OPEN (corrective round).** The round-28 independent review BLOCKED the first fix: bypassed browser evidence (Runtime.evaluate'd source + direct route calls), a cancellation regression, no startup generation sync (post-reload invokes failed closed), an active-tab flow that enrolled the NTP, blind window.* inference, forgeable status, duplicate listeners on re-enroll, stale tools never removed, and partial injection reported as success. The corrective round lands: bridge startup enrollment sync (`enrollment.status`), the explicit tab picker (`agent.discoverable-tabs` + exact tabId threading), positive opt-in inference (`window.webmcpExpose`) + source-threaded dispatch (declared via modelContext, inferred via the captured registry — never window[name]), strict schema rejection, in-flight cancellation restored, versioned singleton teardown in both worlds, complete ordered replacement snapshots (incl. empty → stale removal), per-tab per-role injection readiness, SW-attested vs page-reported status separation with bounded fields + redacted page errors. Proof: `scripts/webmcp-acceptance.ts` 32/32 on the production path (real UI clicks, scriptParsed, console lifecycle, visible side effect before/after reload + navigation). **Residual: the OS-level permission-prompt gesture is unattested headless — `deno run -A scripts/webmcp-acceptance.ts --headed` completes it (docs/WEBMCP-ACCEPTANCE.md).**

## In flight (2026-08-17)
- **WebMCP discovery headed attestation** — the corrective round is code-complete + production-path-tested; the one residual is the manual permission-prompt gesture (see the OPEN item above + docs/WEBMCP-ACCEPTANCE.md).
