...
ACP command discovery and insertion-only composer picker are candidates in `wp7y`,
dependent on harness-tools `430b76fa`. Native dispatch (`6yfm`) and retained
session lifecycle (`v05y`) remain separate; insertion is not execution.
...
- **WebMCP discovery — passive registry acceptance candidate in review.** The owner rejected
  ...
  http(s) page, persist a 100-origin / 24-hour LRU registry, and make every
  `agent.discoverable-tabs` caller require a recent positive capability report for the
  current browser-attested tab/document. Zero-tool snapshots remove entries. The repaired
  production-path journey is **37/37 pass**: a WebMCP fixture enters the picker before
  enrollment, then the existing exact-tab authenticated enrollment and invocation path is
  exercised through reload and navigation.
