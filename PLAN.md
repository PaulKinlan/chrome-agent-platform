...
The public lazy shadow is loaded-MV3 verified. Source-only OPFS, bundled-package and retained code-diff authorities own strict records and bounded retention without execution or mutation routes. Canonical metadata covers all exact 9 browser + 29 management tools—namespaced capabilities, backing optional permissions, grant scope, replay, gesture, mutation and route family—and adds selected-only bounded summaries/digests plus a non-selected count to the Settings capture. Browser/management dispatch and validators are unchanged; every descriptor remains cataloged, including the documented unsafe-for-cutover list, but none is provider-exposed. The independently reviewed exploratory MV3 runtime probe passed Gate 0. Gate 1 remains source-only: an unreachable synchronous WASI Preview 1 table over injected memory/workspace adapters, with exact import, rights, path, quota, cancellation and errno contracts but no Worker, route, OPFS construction, package bytes, instantiation or execution. After review: offscreen fresh-Worker integration; provenance-clean bundled tools; a separately owner-approved diff mutation lifecycle; loaded denial/revoke/race evidence before any Chrome lazy cutover; Tool Library; owner install only after distribution-policy resolution; spreadsheet tools; continuous abuse gates. Providers still receive the eager set and cannot call `search_tools` or `execute_tool`; provider cutover remains separate. The existing semantic-search task remains the sole semantic authority and is P0; embeddings/SQLite are not selected by the lexical slice.
...
`CAP-FB-20260822-PACKAGE-ARCHIVE-FRESHNESS-01` is public at exact `1fd65c6` and reopened narrowly for deterministic build-marker review. Production ZIPs are assembled from Git's tracked extension inventory plus the current generated dist and byte-identical generated changelog—never a wholesale local-tree copy. A fresh same-directory temp ZIP is exact-inventory/hash verified before atomic replacement, so ignored artifacts and removed files cannot survive from local state or an older final archive. The successor removes per-run owner/time data from `dist.complete`; canonical marker bytes bind the exact Git commit, indexed source digest, and generated bundle hashes. Packaging validates the binding before and after inventory hashing, making same-source builds byte-identical while stale/legacy markers fail closed.
