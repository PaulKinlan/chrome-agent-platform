...
- **Scheduled-memory key-bound flood correction (`CAP-FB-20260821-SCHEDULED-MEMORY-QUOTA-01`)** — urgent local successor isolates the retained Durable registry and each execution's record/log/outbox/resume/payload authority from owner/model master memory without changing the 500-key/store, 8 MiB/store, 64 MiB global, or 256 KiB/value ceilings. Boot performs idempotent copy-verify-version-delete migration of legacy authority only. Exact key-bound alarm failures disarm once and remain visible with Retry/Cancel instead of logging on every tick. Independent source review and a short loaded-extension migration/retry check remain pending.
