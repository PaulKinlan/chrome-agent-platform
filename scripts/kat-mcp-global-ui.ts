...
const EVIDENCE = Deno.env.get("CAP_EVIDENCE_DIR") ?? durableDir("cap-mcp-global-ui-kats");
...
async function main() {
  ...
  // Require the SDK-backed remote mount to be bundled into the SW (the direct
  // import from lib/mcp-client.js that both the Test-connection route and the
  // per-run tool injection use). The StreamableHTTP transport is the tell.
  try {
