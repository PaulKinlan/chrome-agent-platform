...
// The ONLY technically-admitted bundled package: cap.bundled.csvtool exposes a
// Settings-only bounded preview (explicit owner click). Every other lane stays
// admitted:false / disabled:true — no catalog/provider selection authority.
const SETTINGS_PREVIEW_LANES = new Set(["csvtool"]);
...
for (const pkg of packages) {
  ...
  const manifest = {
    ...
    meta: { category: String(meta.category), channel: "bundled", description, label: pkg.toolId, status: meta.metaStatus ?? (SETTINGS_PREVIEW_LANES.has(pkg.lane) ? "settings-preview-enabled" : "disabled-no-host"), note: meta.metaNote ?? `evidence: /tmp cap-fixed-tools/${pkg.lane}; owner decision CAP-DECISION-TEMPLATE-20260822-06` },
  };
  ...
  const settingsPreview = SETTINGS_PREVIEW_LANES.has(pkg.lane);
  descriptorRows.push({
...
`);
writeFileSync(join(REPO, "packages/bundled/README.md"), `# Bundled tool packages (immutable; csvtool settings-preview only)
