...
// the 19 tools in SETTINGS_PREVIEW_LANES expose bounded Settings-only previews
// (explicit owner click). Every other lane stays admitted:false / disabled:true
// — no catalog/provider selection authority.
const SETTINGS_PREVIEW_LANES = new Set(["csvtool", "uuid", "head", "tail", "cut", "base64", "md5sum", "sha256sum", "sha512sum", "wc", "xxd", "sort", "uniq", "tr", "grep", "toml2json", "markdown", "diff", "patch"]);
...
`);
writeFileSync(join(REPO, "packages/bundled/README.md"), `# Bundled tool packages (immutable; 19-tool Settings-preview tranche)
