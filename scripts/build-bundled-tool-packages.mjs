...
// the 17 tools in SETTINGS_PREVIEW_LANES expose bounded Settings-only previews
// (explicit owner click). Every other lane stays admitted:false / disabled:true
// — no catalog/provider selection authority.
const SETTINGS_PREVIEW_LANES = new Set(["csvtool", "uuid", "head", "tail", "cut", "base64", "md5sum", "sha256sum", "sha512sum", "wc", "xxd", "sort", "uniq", "tr", "grep", "toml2json", "markdown"]);
...
for (const pkg of packages) {
  ...
  const FILE_READ_DECLARED_CAVEAT =
...
`);
writeFileSync(join(REPO, "packages/bundled/README.md"), `# Bundled tool packages (immutable; 17-tool Settings-preview tranche)
...
- Descriptors (\`extension/lib/bundled-tool-packages.data.js\`): ONLY the static
  ...
  \`uniq\`, \`tr\`, \`grep\`, \`toml2json\`, \`markdown\` are
  ...
  authority). The other 9 remain \`admitted:false\`, \`disabled:true\` (reason
