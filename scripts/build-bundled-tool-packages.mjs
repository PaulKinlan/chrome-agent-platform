...
// The technically-admitted Settings-preview allowlist (the static tranche):
// csvtool + uuid + head + tail + cut expose bounded Settings-only previews
// (explicit owner click). Every other lane stays admitted:false / disabled:true
// — no catalog/provider selection authority.
const SETTINGS_PREVIEW_LANES = new Set(["csvtool", "uuid", "head", "tail", "cut", "base64", "md5sum", "sha256sum", "sha512sum", "wc", "xxd"]);
...
`);
writeFileSync(join(REPO, "packages/bundled/README.md"), `# Bundled tool packages (immutable; 11-tool Settings-preview tranche)
...
(inventory sha256 df352d9adef4cab494e80ae0518a46be234622a7ac8e1b224d4b43dc55e8c6ad).
...
- Descriptors (\`extension/lib/bundled-tool-packages.data.js\`): ONLY the static
  ...
  \`md5sum\`, \`sha256sum\`, \`sha512sum\`, \`wc\`, \`xxd\` are
  ...
  authority). The other 14 remain \`admitted:false\`, \`disabled:true\` (reason
