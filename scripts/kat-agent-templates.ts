...
// Lanes run KATs in parallel — a fixed debug port attaches to ANOTHER lane's
// browser (the silent wrong-target failure). Probe for a free port per run.
async function freePort(preferred: number): Promise<number> {
  for (let p = preferred; p < preferred + 50; p++) {
    try { const l = Deno.listen({ port: p }); l.close(); return p; } catch { /* in use */ }
  }
  throw new Error("no free debug port");
}
const PORT = await freePort(9359);
