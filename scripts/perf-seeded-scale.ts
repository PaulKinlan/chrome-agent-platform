...
// growth check are reported but not counted until RUN-LOG-COMPACTION-01 lands:
// what remains of their latency is durable-runs enumerating every execution
// store (`store.keys()` per execution in extension/lib/durable-runs.js), a
// different owner. Flip SOFT to false in that lane to make them hard.
//
...
const SOFT = true; // until CAP-FB-20260830-RUN-LOG-COMPACTION-01 bounds the execution enumeration
...
function check(name: string, cond: boolean, detail?: unknown, soft = false) {
  ...
  else if (soft) { warn++; console.log(`WARN (soft, pending RUN-LOG-COMPACTION-01): ${name} — ${JSON.stringify(detail)}`); }
...
try {
  ...
  check(`seeded: agent.run at ${TARGET} threads within 1.5x of the first ten runs (no O(runs^2) growth)`, last.runP50Last10 <= Math.max(1.5 * last.runP50First10, 60), { first10: last.runP50First10, last10: last.runP50Last10 }, SOFT);
  ...
  await Deno.writeTextFile(`${OUT}/seeded-scale.json`, JSON.stringify({ ext: EXT, target: TARGET, rows, runMs }, null, 1));
