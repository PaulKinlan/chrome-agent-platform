...
//   §6  orphaned alarms (a deleted agent's recipe:<slug> schedule) are
...
try {
  ...
  check("contract §6: schedule.cancelOrphans cancels the orphaned alarm", cancelled?.ok === true && Array.isArray(cancelled?.cancelled) && cancelled.cancelled.includes("recipe:orphan-kat"), cancelRes);
