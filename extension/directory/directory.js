 ...
 *
 * The source is `agent.tool-offers`, the SAME permission-free route the hub's
 * own chip reads, not `agent.discoverable-tabs`: that route refuses with
 * `needScripting` until the owner has granted the optional `scripting`
 * permission, and a Directory that renders nothing in that state is the exact
 * silent-dead-end the reported bug is about. The offers route is the passive
