...
// ── THE SCHEMA-2 SURFACE IS INJECTED, NEVER IMPORTED (ycez / ltkj.2) ─────────────────────────────
// The Emscripten schema-2 validator and the typed-graph auditor must run in the OPTIONS document,
// never in the service worker: lib/tool-exec-preview.js makes THIS authority reachable from
// background/service-worker.js, so a static import here put both modules into the SW bundle —
// measured 3,009,120 against the unchanged 3,000,000 Store budget (26,243 bytes of reachable code).
// The broker (lib/emscripten-admission.js, reached only from the options document) registers the
// surface; without it a schema-2 record REFUSES BY NAME rather than validating with a partial
// implementation. Schema 1 — everything the Store ships today — never needs it.
let schema2Surface = null;

/** Register the schema-2 validation/audit surface. Called by lib/emscripten-admission.js. */
export function registerSchema2Surface(surface) {
  if (!surface || typeof surface.validateManifest !== "function" || typeof surface.auditGraph !== "function") {
    fail("schema2_surface_invalid", "the schema-2 surface must provide validateManifest and auditGraph");
  }
  schema2Surface = surface;
}

/** The registered surface, or a NAMED refusal — never a silent partial validation. */
function schema2OrFail() {
  if (!schema2Surface) {
    // NOTE for anyone editing this message: do NOT write the broker's path here. The reachability gate
    // reads path-shaped STRINGS as dependencies (comments are excluded), so naming the module in a
    // runtime message makes the service worker "reach" it on paper while esbuild does not bundle it —
    // the pin in tests/reachability.test.ts caught exactly that on the first run. #broker-module
    fail("schema2_broker_unavailable", "schema-2 validation runs in the options document; load the admission broker there");
  }
  return schema2Surface;
}
...
function validateManifestObject(manifest) {
  ...
  if (schema2) schema2OrFail().validateManifest(manifest);
  return manifest;
}
...
export class WasmPackageAuthority {
  ...
  _record(validated, measured, previous = null) {
    ...
    const schema2 = manifest.schemaVersion === 2;
    const graphIdentity = schema2 ? schema2OrFail().identity(manifest) : null;
